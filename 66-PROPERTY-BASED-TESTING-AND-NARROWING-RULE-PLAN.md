...
- **[VERIFIED — INVALIDATES v1 trigger] `/spec-check` is opt-in advisory** (plan 62 D14; bracketed in the `src/CLAUDE.md` workflow chain). A property lane conditioned on "plan-62 IR non-empty" fires only when the user opted into `/spec-check` — most features skip it, so the v1 Option-A trigger reproduces Kiro's evaporation one level up. The trigger must not depend on `/spec-check` having run. **(AMENDED 2026-08-19 — the PREMISE of this bullet has changed and the CONCLUSION has not been re-argued.** `82-SPEC-CHECK-SUBJECT-RESOLUTION-MANDATORY-PLAN.md` amended plan 62's D14 in place: `/devforge:spec-check` is now run-mandatory — `/devforge:plan` blocks until a fresh report exists — so "most features skip it" no longer holds. **This does NOT revive the v1 Option-A trigger**, because the FIRST verified bullet above is untouched and independently fatal: the IR still does not persist past the run's scratch dir, so there is no non-empty IR for any lane to read. Anyone reviving Option A must clear that bullet first, and must re-argue this one rather than cite it. Phase 5 stays deliberately absent.**)
...
- Plan 62 (SMT requirements consistency) — sibling, NOT a dependency anymore: its IR is scratch-only and `/spec-check` is opt-in (see Verified state). Becomes a dependency only for the deferred stub-generation follow-on. **(AMENDED 2026-08-19 — the second half of that reason no longer holds, and the conclusion has not been re-argued.** `82-SPEC-CHECK-SUBJECT-RESOLUTION-MANDATORY-PLAN.md` amended plan 62's D14 in place: `/devforge:spec-check` is no longer opt-in — it is user-typed and `/devforge:plan` blocks until a fresh report exists — as recorded in the amended `[VERIFIED]` bullet under **Verified state** this line points at. **This does NOT revive the v1 Option-A trigger**, because the scratch-only-IR premise is untouched and independently fatal: the IR still does not persist past the run's scratch dir, so there is no non-empty IR for any lane to read. Anyone reviving Option A must clear that premise first, and must re-argue this line rather than cite it. Phase 5 stays deliberately absent.**)**
...
The trap in WI-1 is building the stub generator first because the IR makes it look easy — it is now VERIFIED to be worse than v1 feared: the IR does not even persist past the `/spec-check` run, and `/spec-check` itself is opt-in, so a generator would have no durable input for most features. **(Amended 2026-08-19: the second clause is dead — `/spec-check` is no longer opt-in, it runs for every feature that reaches `/devforge:plan`. The FIRST clause is untouched and on its own carries the trap: the IR still does not persist, so a generator still has no durable input for ANY feature — the trap got narrower in its reason and no weaker in its force.)** The gate (`verify-property-coverage` on architect-named targets) is the part with teeth; generation is a follow-on plan. Kiro had the generation idea and no gate, and shipped zero property tests — that ordering is the whole lesson. WI-2 is deliberately tiny: one paragraph of constitution text plus a `/plan` touchpoint plus the canonical-defaults sync; do not let it grow a detector it doesn't need yet.
