...
## Workstream B + C-onboard — SHIPPED 2026-06-21 (working tree, uncommitted at write time)

**B (flow fix)** + **C (onboard retirement)** done together (they share the `src/CLAUDE.md` flow edit). `/setup-wizard` retirement split out to `30-RETIRE-SETUP-WIZARD-PLAN.md` — it still hosts the `generate-agents.py` spec source + is the documented agent-install entry point, so it can't be deleted without a migration.

Onboard retirement (verified — emitter test 9 OK, generate_docs_helper 321 OK, scratch `install.sh` exit 0 with no onboard command/helper emitted + agents still generated):
- Deleted `src/commands/onboard/` + `src/devforge/lib/onboard_helper{,.py}`.
- `scripts/emitters/claude.py`: removed `onboard` from `_PROMOTED` + rewrote the docstring retirement note.
- `tests/scripts/test_claude_emitter.py`: swapped the `onboard` spot-check for `constitute`.
- `src/manifest.json`: removed the `onboard.md` + dangling `_tech-writer-onboarding.md` entries.
- `src/agents/tech-writer.md`: removed "Onboarding Mode" (now two modes: Normal + Skeleton-Fill) — instruction-author + instruction-reviewer.
- `src/agents-AUTHORING.md`: "tech-writer's 3 operating modes" → 2 (review finding F1).
- `src/CLAUDE.md`: flow → 2.0 chain (`/init-forge → /generate-docs → /configure → /constitute → …`); removed the `/onboard` catalog entry; ADDED a `#### /generate-docs` catalog entry (review finding F2 — the awareness gap left by removing onboard).
- `src/docs/{overview,architecture}.md`: `onboard` → `generate-docs` in the stub-template prose.
- `install.sh` + `update.sh`: dropped `onboard_helper` / `onboard` from comments + the re-emit log string.
- `CHANGELOG.md`: Added (single-root) + Removed (onboard) entries.
- README.md + DEVELOPMENT-STATUS.md: removed the onboard command bullet / Onboarding-System section / transition rows; setup section + transitions table moved to the 2.0 chain.

KNOWN residual (tracked, not this plan): `tests/lib/test_generate_docs_helper.py` keeps a cosmetic `.onboard-state.json`-distinctness assertion (harmless, still passes); README + DEVELOPMENT-STATUS carry broader pre-2.0 staleness (`/execute-task`, setup-wizard prose) — see plan 30's "Related debt".

---

### (original B scope, now done above) — fix the stale 2.0 flow in `src/CLAUDE.md` (consumer overlay)
