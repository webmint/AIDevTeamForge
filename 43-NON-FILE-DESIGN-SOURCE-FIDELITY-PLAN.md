...
**Status:** **RATIFIED 2026-06-25 (Option 4 + Option 1 hybrid) — Steps 1A + 1B + 1D SHIPPED (working tree), Step 1C GATED on plan 42, consumer e2e remaining (user-driven)** on `develop-2.0-init`. See `## Ratified` for the maintainer decision + OQ resolutions and `## Phase 1` for the per-step build breakdown. Shipped: the collision-free `design_source` producer (`/specify` Phase 4 capture + `set-design-source` setter + `**Design source**:` render line) + the classify/WARN helper (`design_helper check-design-source`) + docs reconcile; built behind python-engineer→python-reviewer + instruction-author→instruction-reviewer + claude-code-guide loops (design + specify suites green; 1003-test cross-suite sweep clean). Step 1C (the `/breakdown` PHASE 2.5 WARN wire-in) is GATED on plan 42 shipping (shared edit region) — NOT yet landed. This file began as a DECISION RECORD capturing a coverage boundary of the design-fidelity apparatus; the decision space + recommendation are retained below for rationale.
...
### Step 1C — `/breakdown` PHASE 2.5 WARN wire-in (GATED on plan 42 shipping)

Built via **instruction-author → instruction-reviewer + claude-code-guide** (ships into `.claude/`). **Do not start until plan 42's `/breakdown` PHASE 2.5/3.5 edits are committed** — this edits the same detect-step region (`src/commands/breakdown/main.md:272`).

- In the PHASE 2.5 detect step, after the `test -f design/reference.html` branch: when the reference is ABSENT, run `design_helper check-design-source --spec <spec.md> --workspace-root .` and, if it emits a WARN on stderr, copy that stderr VERBATIM into the user-facing message before proceeding to Phase 3. This replaces the current silent skip for the declared-non-file-source case; a genuine `none`/non-UI feature still skips silently (the verb emits nothing).
