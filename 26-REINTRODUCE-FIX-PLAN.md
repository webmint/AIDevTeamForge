...
**Status**: DRAFTED 2026-06-18, not started, on `develop-2.0-init`.
...
NO change to `/implement`, `implement_helper`, or any back-half verb. `/fix` is a SECOND CALLER of these binaries, exactly as `/verify` is a second caller of `verify-touched`.
...
`/fix` CALLS the existing `implement_helper` verbs (`verify-touched`, `merge-review-panel`, `run-forcing-functions-gate`, `wip-commit`); it never re-implements them. Because those are single-source-of-truth binaries, a caller of them cannot drift from them — this is the structural answer to plan 21 §1's "kept in sync by hand" staleness objection (which was true of the v1.28 COPY draft, not of a thin caller). Do NOT resurrect or port the deleted v1.28 `fix.md` draft.
...
- **Changing `/implement`, `implement_helper`, or any back-half verb** (D3/D6 — `/fix` is a second CALLER of the unchanged binaries).
...
2. **No blocking OQ before Phase 0** — OQ-1/OQ-2/OQ-4 (the verb) resolve at Phase 1, OQ-3 at Phase 4, OQ-4 (how the rule invokes the verb) at Phase 3b. Execute Phase 0 (the doc-only decisions + the plan-21-D1 back-pointer) first so the supersession is on record before any code lands.
3. **Execute Phases 0→5 in order** (each green before the next). Phase 0 is the supersession record; Phase 1 builds the `_fix/` subpackage (incl. the `in-fix-window` window gate); Phase 2 writes `main.md` + references; Phase 3 wires the `/review` + `/verify` command-body offers (cases 1–2); Phase 3b wires the case-3 conversational offer as a TIGHT always-on `src/CLAUDE.md` rule; Phase 4 wires it into the emitter + reconciles docs (incl. the D4 `storage-rules.md` re-point); Phase 5 is the user-driven HARD GATE.
