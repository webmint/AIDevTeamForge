...
**Status**: DONE (build) 2026-06-19 on `develop-2.0-init` — Phases 0–4 built, reviewed, and committed (`efa16e4`, `154c2c4`, `f91e1b4`, `cd7c688`, `26ae580`); install ride passed (fix command emits, executable `fix_helper`, 0 placeholder leaks). **Phase 5 (testForge20 e2e) WAIVED by maintainer 2026-06-19 — NOT executed.** `/fix` is build-verified (unit tests + install ride) but not runtime-e2e-validated; opportunistic e2e validation deferred. **D2 amended in place 2026-09-03 by plan 93 (`93-MODEL-INVOCATION-CARVE-OUT-NARROWING-PLAN.md`): `/devforge:fix` is model-invocable; the "never auto-invoked" clause is restated as "run only on the user's agreement to an offer, never on the model's own initiative".**
...
**(AMENDED 2026-09-03 — plan 93.)** The sentence *"The model NEVER auto-invokes `/fix` (every forge command sets `disable-model-invocation: true`)"* was true as written — every command then carried the flag — and is now false on both halves: plan 63 removed the flag from thirteen commands on 2026-08-07, and plan 93 removed it from `/devforge:fix` on 2026-09-03 (the four setup commands keep it). D2's boundary SURVIVES in substance — `/devforge:fix` is OFFERED in the three in-window situations (plus plan 88's cold lane), consumes written findings, never accepts free text, never self-initiates — and the "never auto-invoked" clause is restated as: run only on the user's agreement to an offer, never on the model's own initiative; on agreement the orchestrator runs it rather than asking the user to type it (plan 93 D2, one agreement per command). `disable-model-invocation` was the CARRIER of that clause, not its reason; the reason lives in the offer rubric and the command's own two-stage hard gate.
