...
**Status**: DRAFTED 2026-06-07, not started. DESIGN ONLY — every step below is a future implementation behind the framework's python-engineer→python-reviewer (helpers/schemas) and instruction-author→instruction-reviewer (spec docs) loops. No code in this file.
...
- **Research handoff `Intent`** (`src/devforge/lib/_research/handoff_schema.py:254-265`) has exactly three fields: `symptom_summary`, `desired_summary`, `scope`. There is NO verbatim-prompt field and NO `suspected_cause`/hypothesis field. So `desired` is already separated from `symptom`, but a *mechanism guess* has nowhere to live except leaking into `desired_summary` or into the recommended-approach summary in `plan_seeds`.
- **Discover handoff `Intent`** (`src/devforge/lib/_discover/handoff_schema.py:209-223`) has `feature_concept`, `topic`, `topic_slug`, optional `scope_summary`. Also NO verbatim-prompt field.
...
4. **Build in dependency order**: 1, 2 (foundations) → 5 (capstone, consumes 1+2) → 6 (enforces 5) → 3, 4 (independent backstops). Each step is gated by its own `### Verify` block + the framework's reviewer loops (python-engineer→python-reviewer for helpers/schemas; instruction-author→instruction-reviewer for spec docs).
5. **Every step is DESIGN here** — implementation is future work. The `### Verify` blocks describe how to confirm a step is DONE once built, not commands to run against the current tree.
6. **Carry the trip-wires as regression assertions.** Steps 2, 3, 4, 5 each name a concrete testForge20-derived trip-wire (the suspected-cause sentence, the §5↔§6 contradiction, the OOS-reaching decision rationale, the minimality interpretation). When building, wire each as a test so the original over-solve cannot recur silently.
