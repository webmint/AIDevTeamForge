...
- `CONSTITUTION-FORCING-FUNCTIONS-PLAN.md` — DRAFTED 2026-05-21. Consumer-side family of mechanical detectors that back constitution rules LLMs systematically violate. Phase 0 substrate + Phase 1 `verify-magic-enum` pilot (string literal duplicates generated-enum member; backs universal §3.5 "no magic values"). Phase 2 empirical-verify gate. Phases 3-4 sketched (`verify-cross-layer-imports` backs §3.6; `verify-any-leak` backs §3.5) — do not implement until Phase 2 passes. Phase 5 wires verify-gate into `/execute-task`. Extends drift-detector pattern; ships consumer-side via `.devforge/lib/constitute_helper`. 5-iter instruction-reviewer loop clean. No code landed yet.
...
- `DISCOVER-HANDOFF-PLAN.md` — IN-FLIGHT 2026-05-20. Steps 1+3+4+5+6+7 SHIPPED on `develop-2.0-init` (commits `895eb80`, `067c09d`, `8e4c8e9`); Steps 2 N/A (research-side reuse); Step 8 = manual testForge20 e2e pending. 786 tests pass + 2 skipped + 13 subtests. Adds `_discover/handoff_schema.py` (17 dataclasses, dataclass mirror of research-side schema), `_discover/_cmds_handoff.py` (finalize-handoff + append-outcome), `_discover/_handoff_build.py` (state→dataclass mapper); extends `_specify/_cmds_handoff.py` (`import-handoff` + `find-handoffs` kind-dispatch) and `_research/_cmds_handoff.py:check-outcome` (kind-dispatch). `/execute-task` outcome-reminder wire-in DEFERRED (command does not exist).
