...
- `01-CONSTITUTION-FORCING-FUNCTIONS-PLAN.md` — IN-FLIGHT 2026-05-21. Consumer-side family of mechanical detectors that back constitution rules LLMs systematically violate. **Phase 0 substrate SHIPPED** (`ea18cd1` on `develop-2.0-init`; 30 substrate tests + `_forcing_functions/_shared.py` Finding/exits/emit_findings/has_inline_escape/path_in_allowlist + `_schema.py` `forcing_functions` top-level key). **Phase 1 `verify-magic-enum` pilot SHIPPED** (pending commit on `develop-2.0-init`; inventory parser + scanner + CLI verb backs universal §3.5 "no magic values"). Phase 2 empirical-verify gate (against testForge20 + cse-strata-ws-forge wrapper) = next manual orchestrator step. Phases 3-4 sketched (`verify-cross-layer-imports` backs §3.6; `verify-any-leak` backs §3.5) — do not implement until Phase 2 closes clean. Phase 5 wires verify-gate into `/execute-task`. Extends drift-detector pattern; ships consumer-side via `.devforge/lib/constitute_helper`. 5-iter instruction-reviewer + 4-iter python-reviewer loops on Phase 0; Phase 1 review loop in flight.
- `02-PLAN-COMMAND-REDESIGN-PLAN.md` — CODE-EXISTS-UNTRACKED 2026-05-21. `/plan` slash command + `plan_helper.py` (989 lines) + `test_plan_helper.py` (71 tests passing) all present at `src/commands/plan/`, `src/devforge/lib/plan_helper.py`, `tests/lib/test_plan_helper.py` but **never committed**. `src/agents/architect.md` still claims `/plan` ownership (3 refs at lines 3, 22, 138) — re-scope per plan Step 2 not verified. Pending: emitter wire-in (`scripts/emitters/claude.py _PROMOTED`), testForge20 install smoke, commit chain on `develop-2.0-init`. Gates THREE-LAYER Gap B+C (now archived).
...
- `04-PR-REVIEW-PLAN.md` — Steps 1-11 SHIPPED 2026-05-20 on `develop-2.0-init`. Adds `/pr-review <PR#>` slash command + `pr_review_helper` subpackage for personal-overlay PR review of foreign repos (forge-overlay-aware AI-slop + blast-radius + scope-drift). Step 12 = testForge20 real-PR validation stop, pending manual e2e. Replay-seeded with DoosanICA/db-cse-ui-strata#304 (MIG-2198).
