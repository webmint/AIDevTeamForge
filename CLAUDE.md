...
- `02-PLAN-COMMAND-REDESIGN-PLAN.md` — CODE-EXISTS-UNTRACKED 2026-05-21. `/plan` slash command + `plan_helper.py` (989 lines) + `test_plan_helper.py` (71 tests passing) all present at `src/commands/plan/`, `src/devforge/lib/plan_helper.py`, `tests/lib/test_plan_helper.py` but **never committed**. **Step 4 (architect re-scope) APPLIED 2026-05-23** — `src/agents/architect.md` now uses invoked-by framing (`You own`=0; `/plan`+`/breakdown`=5 refs, all "invoked by / does not run"); landed via iterative instruction-author→instruction-reviewer loop (clean). **Step 2 tests green 2026-05-23** — the scrubbed third-party 008 fixture was replaced with a fully synthetic "widget catalog" spec rendered at test time via real `specify_helper render`; 71/71 pass. **Step 5 (emitter + install) DONE 2026-05-23** — `"plan"` in `_PROMOTED`; `install.sh` against testForge20 exits 0; `plan.md` + `plan_helper` + re-scoped `architect.md` verified in target (`pick-spec` no-spec → exit 2). Pending: full `/plan` boot e2e (user-side manual), **Step 7 handoff-consumption (no code yet)**, commit chain on `develop-2.0-init`. Gates THREE-LAYER Gap B+C (now archived).
