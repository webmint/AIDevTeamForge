...
- `01-CONSTITUTION-FORCING-FUNCTIONS-PLAN.md` — IN-FLIGHT 2026-05-21. Consumer-side family of mechanical detectors that back constitution rules LLMs systematically violate. **Phases 0+1+2+3+4 SHIPPED** on `develop-2.0-init` (Phase 0 substrate `ea18cd1`; Phase 1 magic-enum pilot `ccc25b8`; Phase 2 empirical-verify gate-relaxation decision `aae5a02` + `EMPIRICAL-VERIFY-MAGIC-ENUM-2026-05-21.md`; Phase 3 cross-layer `a370229`; Phase 4 any-leak `4b058da`). **Phase 5 split**: **5a (pre-commit hook + `/init-forge` wizard wire-in + `/constitute` wizard `forcing_functions` step) = TODO, next-session pickup**; **5b (`/execute-task` verify-gate) DEFERRED → ABSORBED INTO `07-EXECUTE-TASK-REDESIGN-PLAN.md` Phase 7** (do not implement here). Phase 6 docs propagation (`CHANGELOG.md` + repo-root `CLAUDE.md` + `src/constitution.md` §3.5/§3.6 cross-refs + `DEVELOPMENT-STATUS.md`) follows Phase 5a. Ships consumer-side via `.devforge/lib/constitute_helper`. Review-loop totals: Phase 0 = 4 iters / 9 findings; Phase 1 = 3 iters / 11 findings; Phase 3 = 4 iters / 8 findings; Phase 4 = 3 iters / 8 findings. 401 tests pass.
