...
**Status**: **V2 applied + empirically verified (3/3 causes) 2026-05-18.** Fresh /research run on testForge20 duplicate-options topic (`research/2026-05-18-restriction-on-adding-the.md`, 24.9K) identifies `strataFamilyToItemAdapters.ts:5` Math.random assignment as root cause across THREE state surfaces: (a) Finding row tagged `runner-up`, (b) Value Production Sites table with `is_stable: false`, (c) recommended-approach Part 1 with explicit `Math.random` → `partNo + characteristicCode` derivation. Runner-up frame confidence `higher` than primary. Score improvement: V1 patches scored 1.5/3 (2026-05-17, BELOW 04-30 baseline of 2/3) → V2 patches score 3/3. Compared to real fix (`bugfix/MIG-2642` commits `74adb5b17` + `df231933a`): adapter identity preservation ✅, recursive findMatchingConfigurationLine ✅ ("recursively walks configurationItem.subLines"), partNo fallback ✅ (named in adapter-fix derivation), `isSingleInstance` static ⚠ partial (discriminator-family hit via `CoreQuoteLine.isAttachment` cite — real fix added new static instead of reusing canonical). Patch 6 (Phase 2.4d data-flow chain trace) forced adapter end-to-end read; Patch 7 (Phase 2.5 stability axis + production-site setter + check 16 hypothesis-citation gate) forced production-site enumeration into hypotheses + recommended approach. Gaps 6 + 7 CLOSED.

Hardened 2026-05-18 against 6 pre-commit concerns (C1 token cost / C2 write-boundary token list / C3 handler-detection fragility / C4 cyclic dep / C5 multi-production-site / C6 trace_path data_flow accuracy). Addresses 2 unaddressed gaps (Gap 6 + Gap 7) surfaced after RESEARCH-FRAMING-REGRESSION-PLAN Patches 1-5 shipped + scored 1.5 of 3 on empirical verify (BELOW pre-patch 04-30 baseline of 2 of 3).

**PRE-FLIGHT (2026-05-18) — concern C6 resolved empirically**: dispatched `trace_path` with `mode=data_flow` and `mode=calls` against two real chains: (a) `_compute_check_8b_would_fire` in ai-dev-team-forge Python — both modes returned non-empty, identical hop lists; (b) `applyCustomerSelectionFromOrgV2` in db-cse-ui-strata TS — both modes returned IDENTICAL hop lists with same QNs/hops. Vue templates + the actual Strata adapter `strataFamilycharacteristicOptionAdapter` have no Function-level graph edges (Vue files only emit File+Module nodes; the adapter has in_degree=0 / out_degree=0). **Verdict**: data_flow mode in current CBM = calls mode. No incremental signal. **Decision**: Patch 6 spec switches to "always `mode=calls`" — drop the `--trace-mode` arg from `record-data-flow-chain` helper and drop the dual-mode fallback prose. Recovery for Vue-handler symptoms (where BOTH modes return empty because Vue files have no Function nodes) is the heuristic-fragility user-fallback prompt — now becomes the primary recovery path for Vue/template-based presentation-layer symptoms, not a secondary fallback.

**PATCH 7 (2026-05-18) — applied + tests green**: Helper additions: `default_report_state()` adds `value_production_sites: []`; `set-value-semantics` gains `--stable-across-calls true|false|unknown` arg with 4 gates in `cmd_set_value_semantics` (1: required when invariant; 2: unknown rejected on presentation-layer; 3: consumer_chain required when invariant; 4: false requires prior `record-value-production-site` row); new `record-value-production-site` subparser + `cmd_record_value_production_site` handler with `--value`, `--file-line`, `--is-stable` args, rejects `(none)` sentinel, distinct `(value, file_line)` dedupe (multi-site support — same value at different file_lines all append; same file_line at different values also append); verify check 16 enforces hypothesis-citation of unstable-value production sites using word-boundary regex `(?!\d)` after the `:line` suffix (prevents `src/foo.ts:5` from prefix-matching `src/foo.ts:50`); `_render_report_md` extended with stability column on Value Semantics table (non-invariant rows render `"—"` for stability) + new Value Production Sites section. Spec additions: Phase 2.5 in `src/commands/research/main.md` extended with stability-axis prose, 4-gate enforcement list (evaluated in helper's actual order), `record-value-production-site` setter description, structural is-stable distinction (randomization vs deterministic reassignment), and check 16 hypothesis-citation gate with bug-mode scope qualifier + enhancement-mode skip note. Helper tests: 23 new tests (7 stability + 7 production-site setter + 4 check-16 + 2 render + 3 reviewer-finding-driven: prefix-collision, no-production-site state, same-fileline-diff-values); `_build_bug_state` updated to pass `--stable-across-calls true`. Total tests: 267/267 green (baseline 244 + 23 new). Reviewer findings applied: python-reviewer surfaced 5 (false-positive render assertion, substring prefix-collision, bool↔string type inconsistency on `is_stable`, missing same-fileline-diff-values test, missing unstable-no-site test — all fixed); instruction-reviewer surfaced 5 (gate count "Three" vs 4 items, gate order mismatch with helper, check 16 missing bug-mode qualifier, stale (44) docstring count, "rare" phrasing — all fixed). Re-loop instruction-reviewer clean.

**PATCH 6 (2026-05-18) — applied + tests green**: Helper additions: `default_report_state()` adds `data_flow_chain: None`; new `record-data-flow-chain` subparser + `cmd_record_data_flow_chain` handler validate handler_qn + write_boundary_qn non-empty + intermediate_qns is JSON array (empty `[]` valid for direct handler→write-boundary) + each intermediate_qn substring-matches an existing Finding's `relevance` OR `surface` field; verify check 15 fires when bug mode + presentation-layer primary symptom + `data_flow_chain` is null. Spec additions: new Phase 2.4d section in `src/commands/research/main.md` between Phase 2.4c and Phase 2.5 — 5-step sequence (identify handler / identify write-boundary / trace_path mode=calls / read intermediates with first-party + shape-conversion-name filters / persist via setter), with user-fallback prompts on heuristic misses for handler-detection (Step 1), write-boundary-detection (Step 2), and handler-not-a-graph-node (Step 3 Vue/template recovery). Helper tests: 13 new tests in `tests/lib/test_research_helper.py` (9 setter + 4 verify-check-15) plus `_build_bug_state` updated to seed an empty chain; pytest 244/244 green (baseline 231 + 13 new). Reviewer findings applied: python-reviewer surfaced 3 doc/comment findings (all fixed); instruction-reviewer surfaced 4 findings (high: stripped unbacked `.devforge/configure.yaml` override paragraph — `write_boundary_tokens` was not a real configure-helper field; med: corrected over-constraint on `--relevance` vs `--surface`; low: changed `--framing` template to `"primary"`; nit: parenthesized the skip-clause). The Step 2 write-boundary user-fallback was added during the configure.yaml fix to maintain coverage on non-conventional verbs without inventing a config surface.
...
1. **Phase 2.4d spec section** mandates the following sequence for bug-mode reports where the symptom is in a presentation-layer file:
   ...
   - Run `trace_path` from the handler QN to the write-boundary QN with `mode=calls direction=outbound`. Record the full path of intermediate function QNs. **Mode rationale** (concern C6 resolved by pre-flight 2026-05-18): CBM's `mode=data_flow` returns identical results to `mode=calls` for first-party project code (no incremental DATA_FLOWS edges in current CBM); Vue/template files have no Function nodes so neither mode resolves handlers defined in template script blocks. Always use `mode=calls`. Downstream LLM steps must read each intermediate body via `get_code_snippet` to confirm the value of interest actually passes through (call-graph reachability ≠ data-flow reachability) — that's where intermediate-value mutation is detected. **Handler-not-a-graph-node fallback** (Vue / template / SFC presentation files): if `trace_path` returns empty OR the handler QN doesn't resolve via `search_graph` (Vue templates only emit File+Module nodes), fall through to the user-fallback prompt for the chain — same prompt as the heuristic-fragility fallback, extended to cover the missing-graph-node case: *"I couldn't trace from `<handler>` to a write-boundary call via the code graph (Vue/template files often aren't indexed at function granularity). What intermediate functions does the handler call before reaching the persistence call? (list function names or `file:line`)"*. The shape-conversion-name filter (concern C1) still applies to the user-supplied chain.
...
2. **New helper setters**:
   - `record-data-flow-chain --handler-qn <qn> --write-boundary-qn <qn> --intermediate-qns '[<qn>, ...]'` — persists the chain. Required when bug-mode + presentation-layer symptom. (Pre-flight 2026-05-18 dropped `--trace-mode` arg: CBM's `mode=data_flow` returns identical results to `mode=calls` for first-party code, so the dual-mode flag adds no signal.)
   - Helper validates: handler_qn + write-boundary_qn non-empty; intermediate_qns is JSON array (may be empty if direct call); each intermediate QN must have a corresponding Finding row (forces the `record-finding` pre-call per spec).
