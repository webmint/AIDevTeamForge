...
#### Step 3.3.6: Multi-ecosystem detection validation gate (BLOCKING for iteration-mode unlock)

**Why this is BLOCKING**: every empirical validation of `/generate-docs` so far has been against testForge20 (Vue 3 / TypeScript / vite / `db-cse-ui-strata/apps/app-web`). The iteration-mode banner removal (multi-package unlock) MUST NOT happen before validating across ecosystems.

**Scope refinement**: testForge20 already proved scale (~900 source files, dense components/ with 456 entries, 28-32 min wall-clock, 0 failed concerns). Size is NOT the open question. **The open question is detection** — does `/generate-docs` correctly detect what matters in foreign ecosystems? This frames smaller candidates as fully acceptable; the test is per-ecosystem detection coverage, not scale.

**Detection at two layers**:

1. **Mechanical detection (helper-side)** — testable WITHOUT codebase knowledge:
   - Phase 1 manifest detection: language / framework / build-tool from `pom.xml` / `Cargo.toml` / `pyproject.toml` / `go.mod`
   - extract-package-scripts on non-npm manifests: Maven goals, Cargo aliases, pyproject scripts
   - Decomposition gate's "substantive subfolder" heuristic on Java's deep `com/company/module/` nesting, Python's `mypackage/submodule/`, Rust's `src/<module>/`
   - extract-snippet on `.java` / `.py` / `.rs` / `.go` files (whitespace normalization, line-range extraction)
   - validate-package + validate-concern exits

2. **Content detection (LLM-side)** — testable via the **newcomer subjective test** (the user is the target reader BECAUSE they don't know the language):
   - Q1 "What does this codebase do?" → tests concern overview prose
   - Q2 "What are the major pieces?" → tests Public Surface
   - Q3 "If I wanted to find functionality X, where would I look?" → tests tree
   - Q4 "Are there any gotchas / risks?" → tests Hazards
   - Q5 "What does this codebase depend on?" → tests Dependencies
   - The user being unfamiliar with the language is a FEATURE — they're the target /research user role.

**Training-contamination caveat**: famous public repos (apache/commons-lang, NumPy, tonic-build, etc.) are in LLM training. Descriptions for them may pull from training memory rather than pure inference. This contaminates LLM-generated content evaluation. Mitigations:
- Prefer LOW-popularity / niche repos (sub-50 stars) — minimal training inclusion
- Prefer post-training-cutoff repos (created/significantly-updated after January 2026)
- Prefer the user's own private/internal Java/Python/Rust code if available (gold-standard isolation)
- Empirically established: GitHub trending is heavily contaminated. Step 3.3.6 should NOT use trending picks as primary candidates.

**Test suite — detection-focused, smaller-codebases acceptable**:

1. **Java small project** (mandatory). Tests: Maven/Gradle manifest detection, deep package-tree handling, Java-specific `--kind` (interface, annotation, abstract-class), public/private boundary detection.
   - Primary candidate: user's own internal Java codebase if available (best training isolation).
   - Public fallbacks: `CommsenArchive/liferay-maven-sdk` (37 stars, J2EE-era, archived 2013 — small but architecturally representative); `skjolber/json-log-filter` (8 stars, modern utility, low training); `floci-io/floci` (4.3k stars, post-cutoff AWS-emulator, descriptive but recent).

2. **Python small project** (mandatory). Tests: pyproject.toml detection, `__init__.py` canonical-aggregator skip-rule, `__all__` boundary detection.
   - Primary candidate: user's own internal Python codebase if available.
   - Public fallbacks: NumPy submodule (`numpy.fft` or `numpy.linalg` — note: heavily training-contaminated; lower content-test value but high mechanical-detection value); a small post-cutoff Python utility.

3. **Rust small project** (mandatory). Tests: Cargo.toml detection, `mod.rs` / `lib.rs` aggregator skip-rule, `pub` boundary detection, `--kind trait/macro/impl` on open enum (Step 3.3.4).
   - Primary candidate: user's own internal Rust crate if available.
   - Public fallbacks: `io7m-com/certusine` (1 star, ACME client — niche infrastructure, low training); a post-cutoff small crate; older systems crates with terse function names.

4. **Optional 4th candidate** — pick whichever ecosystem feels under-tested after running 1-3 (Go, C/C++, C#, Kotlin, etc.). Optional because the first three should give enough cross-ecosystem evidence.

**Test methodology per candidate (detection-focused)**:

**Mechanical detection checklist** (objective, no codebase knowledge needed):

| Check | Pass criterion | Source |
|---|---|---|
| Manifest detection | Phase 1 returned correct language/framework/build-tool (verifiable by inspecting the manifest yourself) | Run output |
| extract-package-scripts | Returned ≥0 scripts, no exceptions; for ecosystems with conventions (Maven goals, Cargo aliases), at least the conventional defaults present | Helper exit + stdout |
| Decomposition gate | Identified ≥1 substantive concern (matches what a deep `find` of source folders shows) | validate-package exit code 2 with `rule: decomposition` errors |
| extract-snippet | Whitespace-normalized equality holds on a sample of cited ranges | Manual sample |
| validate-package exit | 0 after slot-fill | Run output |
| validate-concern exit per concern | 0 after slot-fill (or ≥6 of 7 concerns clean — partial fail OK) | Run output |
| Citation rate | >95% citations validated | Helper validate output |
| Tree completeness ratio | tree entries match `find <pkg>/<subfolder>/* | wc -l` | Manual check on one concern |
| Wall-clock + tokens | Within reasonable factor of testForge20 baseline (28-32 min, ~70-90k tokens), accounting for codebase size | Phase 5 timing table |

If ANY mechanical check fails → ecosystem-specific spec/helper bug. Iterate, re-test. NOT an architecture failure.

**Content detection (newcomer subjective test)**:

User reads ONLY the produced docs (does not open source). Answers the 5 questions above. Score:
- **4-5 of 5 answered with reasonable confidence** → content detection PASS
- **2-3 of 5** → content detection PARTIAL (architecture gap on this ecosystem)
- **0-1 of 5** → content detection FAIL (architecture insufficient)

**Aggregate decision criterion**:

- **PASS** (unlock iteration mode + Step 3.3.4): all 3 mandatory ecosystems → mechanical PASS + content ≥PARTIAL.
- **CONDITIONAL PASS** (iterate on spec, re-test, then unlock): mechanical PASS on all + content PARTIAL on ≥1 ecosystem. Iterate spec wording until content reaches PASS, then re-test that ecosystem.
- **FAIL** (unlock Step 3.3.7 — per-file docs fallback): mechanical fail (after spec/helper fix attempts), OR content FAIL on ≥1 mandatory ecosystem after spec iteration.

**What to skip / why** (low validation value):
- Modern Spring Boot Java, modern Django Python, modern React/Next.js (descriptive naming, redundant with testForge20)
- Massive monorepos (Kubernetes, TensorFlow, Linux kernel — wall-clock dominator; pick smaller per-package targets instead)
- GitHub trending picks (training-contamination; descriptive-naming bias; skip as primary)
- Educational / tutorial repos (testing the spec on tutorial code is meta-circular)

**Procedure per candidate**:

1. Pick the codebase. Prefer user's private code (best isolation); fall back to public small/niche candidates.
2. Clone (if public) to a sibling directory of the forge repo.
3. Install / setup `/generate-docs` against the candidate's install root (re-run `install.sh`).
4. Run `/generate-docs` (still in iteration mode — single-package; pick most representative package in the candidate's structure).
5. Run mechanical-detection checklist. Each check pass / fail.
6. Run newcomer subjective test. 5-question score.
7. Combine: ecosystem decision (PASS / PARTIAL / FAIL).
8. Repeat for all 3 mandatory ecosystems.
9. Aggregate ecosystem decisions → architecture decision (PASS / CONDITIONAL PASS / FAIL).

**Verify**: empirical assessment writeup added to `GENERATE-DOCS-EXECUTION-LOG.md` with per-ecosystem mechanical-checklist scores + newcomer-test scores + aggregate decision. If PASS, iteration-mode banner can be removed in the same iteration as Step 3.3.4. If CONDITIONAL PASS, iterate spec; re-test the failing ecosystem; re-evaluate. If FAIL, Step 3.3.7 unblocks.
