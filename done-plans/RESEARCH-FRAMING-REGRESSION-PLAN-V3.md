...
**Status**: Patches 8 + 9 LANDED + EMPIRICALLY VERIFIED 2026-05-18 on `develop-2.0-init` (helper + tests + spec; 320 passed via venv-test). Replay on testForge20 splitOnSNA topic in fresh session: V3 framework proposed S3 wrapper-default (Approach A: 2-line edit at OrderViewer.vue:281 signature + :290 drop literal) in turn 1 with zero user iteration — vs. 2026-05-18 baseline that required 3 push-back turns. `literal_archaeology` row recorded for `false` at OrderViewer.vue:290 (commit cca351494, DEAL-292, 2023-12-12) with `--intent forgotten` rather than `inherited-refactor` (LLM saw both fits per primary-hypothesis rationale; per Phase 2.5b per-intent table both intents trigger same fix-layer-escalation recovery rule — functional outcome identical). `proposed_call_shape: fetchOrder()` rendered correctly under recommended approach. Pattern lesson `feedback_literal_archaeology_and_argument_duplication` empirically validated. V4 not required. Hardened 2026-05-18 against 4 pre-commit refinements: (R1) enum locked at 6 values including `generated`; (R2) literal-token regex expanded to cover Python `True|False|None`, hex `0xff`, BigInt `100n`, scientific `1e-9`, backtick templates `` ` `` (non-interpolated); array/object literals documented out-of-scope; (R3) Patch 9 identifier regex updated for optional chaining `a?.b?.c`; (R4) oldest-introducing-commit anchor edge case (deliberate-then-stale) acknowledged + deferred to /audit. Predecessor V2 plan empirically verified 3/3 on Strata duplicate-options bug 2026-05-18. V3 closes 2 unaddressed gaps surfaced on the 2026-05-18 splitOnSNA bug run, where /research recommended a call-site fix that produced argument duplication (`fetchOrder(false)` → `fetchOrder(isExternalUser.value)` while `isExternalUser.value` was already arg 2 in the same call); user required 3 dialogue turns to escalate to S3 (wrapper default-param), and LLM never opened git blame on the hardcoded `false` to surface its historical intent. Patches 6+7 forced WHERE-evidence + STABILITY-evidence; V3 forces HISTORICAL-evidence + FIX-SHAPE-evidence.
