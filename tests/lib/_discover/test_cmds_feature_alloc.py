"""Tests for _discover/_cmds_feature_alloc.py's --ticket wiring.
...
This is the FIRST test file under tests/lib/_discover/ for the
...
legacy monolith tests/lib/test_discover_helper.py (slug-only behaviour, no
--ticket). This file covers ONLY the new --ticket surface and does not
duplicate that file's slug-only cases.

Mirrors tests/lib/_research/test_cmds_feature_alloc.py exactly (the two
_cmds_feature_alloc.py wrappers are near-identical twins by design -- see
that module's own docstring), swapping research_helper.py for
discover_helper.py as the CLI entry point.
...
discover_helper.py, parsing the CLI's actual stdout JSON -- never a
...
def _run_discover(devforge_dir, *args):
    """Invoke discover_helper.py <args> as a subprocess."""
    return subprocess.run(
        [sys.executable, str(_DISCOVER_HELPER_PY), "--devforge-dir", str(devforge_dir)]
        + list(args),
        capture_output=True,
        text=True,
    )
...
class TestRenderBranchCommandTicket(unittest.TestCase):
    ...
    lives in tests/lib/test_discover_helper.py::TestRenderBranchCommandCli
