"""Tests for research_helper set-design-anchor + its finalize-handoff carry
(plan 53 Phase 1 — design intent as a first-class pipeline input).

Covers:
  - set-design-anchor persists {kind, file, selectors} into
    research-state.json (memo.design_anchor.value).
...
Subprocess pattern (matches test_research_helper.py): each test runs in its
own tempfile.TemporaryDirectory, invoking the real research_helper.py CLI.
...
from _research import handoff_schema as rhs  # noqa: E402
from _specify._cmds_handoff import _dict_to_dataclass  # noqa: E402
...
def _read_memo(devforge):
    return json.loads((Path(devforge) / "research-state.json").read_text())


# ---------------------------------------------------------------------------
# set-design-anchor setter.
# ---------------------------------------------------------------------------


class TestSetDesignAnchor(unittest.TestCase):
    def test_html_value_persists_kind_file_selectors(self):
        with tempfile.TemporaryDirectory() as tmp:
            devforge = Path(tmp) / ".devforge"
            r = _run([
                "--devforge-dir", str(devforge), "set-design-anchor",
...
def _build_minimal_bug_state(devforge):
    """Populate the minimal valid bug-mode state finalize-handoff requires.

    Trimmed to exactly the required-field guards in cmd_finalize_handoff
    (mirrors test_research_helper.py's _build_minimal_bug_state_for_handoff).
    """
    _run(["--devforge-dir", str(devforge), "reset-memo"])
    _run(["--devforge-dir", str(devforge), "reset-report"])

    for d, val in (
        ("symptom", "Font renders wrong on card"),
        ("affected-area", "src/components/Card.vue"),
        ("repro-or-current", "Open card view"),
        ("desired", "Font matches the design reference"),
        ("scope", "one function"),
        ("unchanged-behavior", "layout unchanged"),
    ):
        _run([
    ...
    _run(["--devforge-dir", str(devforge), "set-topic", "--value", "font-drift"])
    _run([
    ...
    _run([
        ...
        "--verify-cost", "Low", "--verify-notes", "visual check",
    ])
    _run([
...
class TestFinalizeHandoffCarriesDesignAnchor(unittest.TestCase):
    def test_captured_anchor_round_trips_through_real_producer(self):
        with tempfile.TemporaryDirectory() as tmp:
            devforge = Path(tmp) / ".devforge"
            _build_minimal_bug_state(devforge)
            r = _run([
                "--devforge-dir", str(devforge), "set-design-anchor",
                "--value", "html:design/reference.html",
                "--selectors", '[".fooBar"]',
            ...
            da = rhs.DesignAnchor(**data["spec_seeds"]["design_anchor"])
            self.assertEqual(da.kind, "html")
            self.assertEqual(da.file, "design/reference.html")
            self.assertEqual(da.selectors, [".fooBar"])
    ...
    def test_no_captured_anchor_finalizes_with_empty_default(self):
        ...
        with the empty default when set-design-anchor was never called.
        """
        with tempfile.TemporaryDirectory() as tmp:
            devforge = Path(tmp) / ".devforge"
            _build_minimal_bug_state(devforge)
...
class TestDesignAnchorBackCompatViaRealLoader(unittest.TestCase):
    def test_missing_design_anchor_key_defaults_via_real_loader(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            handoff = _dict_to_dataclass(rhs.Handoff, data)
            self.assertEqual(handoff.spec_seeds.design_anchor, rhs.DesignAnchor())
