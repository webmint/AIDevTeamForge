"""Tests for src/devforge/lib/_review/_preflight.py.
...
Fixture shapes mirror tests/lib/_audit/test_preflight.py (TestPreflightContext)
...
from _review._preflight._UNPOPULATED_SENTINELS.
"""
...
from _review._preflight import (  # noqa: E402
...
class TestSentinelSet(unittest.TestCase):
    """Ensure _review uses the same sentinel set as _audit (parity requirement)."""
    ...
    def test_sentinel_parity_with_audit(self):
        ...
        self.assertEqual(
            ...
            set(audit_sentinels),
            msg=(
                "_review sentinels diverge from _audit sentinels. "
                "Keep them in sync so both helpers enforce the same gate."
            ),
        )
...
class TestPreflightContext(unittest.TestCase):
    ...
    def test_missing_constitution_flagged(self):
        ...
        self.assertFalse(r["setup_chain_ok"])
        labels = r["missing_artefacts"]
        self.assertIn("/devforge:constitute", labels)
    ...
    def test_memory_present_and_excerpt_capped_at_default_excerpt_lines(self):
        # RENAMED (was test_memory_present_and_excerpt_capped_at_40_lines):
        # plan 79 Phase 1 raises the excerpt budget to DEFAULT_EXCERPT_LINES
        # (120) CONTENT lines and makes it section-aware. The old
        # headingless 60-line fixture is 100% preamble under the new
        # algorithm and renders "" -- the old assertLessEqual(len(lines),
        # 40) still "passed" on that empty result, but vacuously: it no
        # longer exercised a cap at all. Re-fixtured with a single real
        # "## " section carrying more lines than the budget, so the cap
        # is genuinely tested again.
        mem_content = "## Lessons\n" + "\n".join(
...
class TestCmdPreflight(unittest.TestCase):
    ...
    def _run_preflight(self, workspace_root):
        """Run review_helper preflight and return (exit_code, stdout_text)."""
        import io
        from contextlib import redirect_stdout, redirect_stderr
        from _review._cli import main
...
class TestCmdCheckStatusAndFlip(unittest.TestCase):
    ...
    def test_empty_to_phase_exits_2(self):
        ...
        self.assertEqual(code, 2)
        self.assertIn("review_helper check-status-and-flip", err)
