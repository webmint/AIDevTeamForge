...
TestCheck5MatchTokenBinding
    match_element_uses_token_passes
    match_element_hardcoded_hex_fails
    match_element_rgb_fails
    match_element_no_match_ref_skips
    spacing_literal_fails_when_scale_available
    spacing_literal_relaxed_when_scale_absent  (OQ-6)
...
from _constitute._forcing_functions._design_tokens._cmd import (  # noqa: E402
    _collect_match_refs,
    _glob_manifests,
...
class TestCheck5MatchTokenBinding(unittest.TestCase):

    def _check(
        self,
        source: str,
        match_refs: Optional[Set[str]] = None,
        spacing_scale_available: bool = True,
    ) -> list:
    ...
    def test_match_element_uses_token_passes(self):
        src = textwrap.dedent("""\
            [data-ref="sidebar"] {
        ...
        """)
        findings = self._check(src, {"sidebar"})
        self.assertEqual(findings, [])

    def test_match_element_hardcoded_hex_fails(self):
        src = textwrap.dedent("""\
            [data-ref="sidebar"] {
              background: #ffffff;
            }
        ...
        findings = self._check(src, {"sidebar"})
        self.assertTrue(len(findings) >= 1)
        self.assertTrue(_has_finding_with(findings, "sidebar"))

    def test_match_element_rgb_fails(self):
        src = textwrap.dedent("""\
            [data-ref="header"] {
              background: rgb(255, 255, 255);
            }
        """)
        findings = self._check(src, {"header"})
        self.assertTrue(len(findings) >= 1)

    def test_match_element_no_match_ref_skips(self):
        # An element not in match_refs is ignored by Check 5
        src = textwrap.dedent("""\
            [data-ref="not-in-manifest"] {
              background: #deadbe;
            }
        """)
        findings = self._check(src, {"sidebar", "header"})
        self.assertEqual(findings, [])

    def test_empty_match_refs_skips_all(self):
        src = textwrap.dedent("""\
            [data-ref="anything"] { background: #fff; }
        """)
        findings = self._check(src, set())
        self.assertEqual(findings, [])

    def test_spacing_literal_fails_when_scale_available(self):
        src = textwrap.dedent("""\
            [data-ref="card"] {
              padding: 16px;
            }
        """)
        findings = self._check(src, {"card"}, spacing_scale_available=True)
        self.assertTrue(len(findings) >= 1)
        self.assertTrue(_has_finding_with(findings, "spacing"))

    def test_spacing_literal_relaxed_when_scale_absent(self):
        # OQ-6: When spacing_scale_available=False, spacing literals are NOT flagged
        src = textwrap.dedent("""\
            [data-ref="card"] {
              padding: 16px;
            }
        """)
        findings = self._check(src, {"card"}, spacing_scale_available=False)
        # spacing violation should not appear
        self.assertFalse(_has_finding_with(findings, "spacing"))

    def test_exact_line_number_for_body_violation_f2(self):
        # F2 regression: body_line_num was line_num + body_line_offset + 1
        # (off-by-one).  The violation must land on the ACTUAL source line.
        src = textwrap.dedent("""\
            [data-ref="sidebar"] {
              background: #ffffff;
            }
        """)
        # Line 1: selector, Line 2: background: #ffffff;, Line 3: }
        findings = self._check(src, {"sidebar"})
        self.assertTrue(len(findings) >= 1, "Expected a finding for hardcoded hex")
        self.assertEqual(findings[0].line, 2,
                         "Violation must be on line 2, not off-by-one")

    def test_exact_line_number_deeper_body_f2(self):
        # Violation is on the 3rd property — must report line 4.
        src = textwrap.dedent("""\
            [data-ref="card"] {
              color: var(--color-text);
              border: var(--border);
        ...
        """)
        findings = self._check(src, {"card"})
        hex_findings = [f for f in findings if "#ff0000" in f.summary]
        self.assertTrue(len(hex_findings) >= 1)
        self.assertEqual(hex_findings[0].line, 4)
...
# ---------------------------------------------------------------------------
# TestManifestGlob  (unit tests for _cmd.py glob helpers)
# ---------------------------------------------------------------------------

class TestManifestGlob(unittest.TestCase):
    """Unit tests for _glob_manifests and _collect_match_refs."""
    ...
    def test_glob_finds_no_manifests_when_specs_empty(self):
        # specs/ dir exists but has no design-manifest.json files
        (self._root / "specs").mkdir()
        found = _glob_manifests(self._root)
        self.assertEqual(found, [])

    def test_glob_finds_no_manifests_when_no_specs_dir(self):
        # No specs/ directory at all
        found = _glob_manifests(self._root)
        self.assertEqual(found, [])

    def test_glob_finds_single_manifest(self):
        self._write_manifest("001-feat", [{"data_ref": "hero", "disposition": "MATCH"}])
        found = _glob_manifests(self._root)
        self.assertEqual(len(found), 1)
        self.assertTrue(str(found[0]).endswith("design-manifest.json"))

    def test_glob_finds_two_manifests(self):
        self._write_manifest("001-feat-a", [{"data_ref": "hero", "disposition": "MATCH"}])
        self._write_manifest("002-feat-b", [{"data_ref": "card", "disposition": "MATCH"}])
        found = _glob_manifests(self._root)
        self.assertEqual(len(found), 2)

    def test_glob_results_are_sorted(self):
        # Sorted order must be deterministic (alphabetical by path)
        self._write_manifest("002-b", [{"data_ref": "b", "disposition": "MATCH"}])
        self._write_manifest("001-a", [{"data_ref": "a", "disposition": "MATCH"}])
        found = _glob_manifests(self._root)
        self.assertEqual(found, sorted(found))

    def test_load_manifest_from_json_match_case_insensitive(self):
        # disposition "match" (lower) and "MATCH" (upper) both work
        p = self._root / "m.json"
        p.write_text(json.dumps({"elements": [
            {"data_ref": "A", "disposition": "match"},
            {"data_ref": "B", "disposition": "MATCH"},
            {"data_ref": "C", "disposition": "SKIP"},
        ]}))
        refs = _load_manifest_match_refs_from_json(str(p))
        self.assertIn("A", refs)
        self.assertIn("B", refs)
        self.assertNotIn("C", refs)

    def test_load_manifest_from_json_empty_elements(self):
        p = self._root / "empty.json"
        p.write_text(json.dumps({"elements": []}))
        refs = _load_manifest_match_refs_from_json(str(p))
        self.assertEqual(refs, set())

    def test_load_manifest_from_json_missing_file_returns_empty(self):
        refs = _load_manifest_match_refs_from_json(str(self._root / "nonexistent.json"))
        self.assertEqual(refs, set())

    def test_load_manifest_from_json_malformed_returns_empty(self):
        p = self._root / "bad.json"
        p.write_text("not json {{{")
        refs = _load_manifest_match_refs_from_json(str(p))
        self.assertEqual(refs, set())

    def test_collect_explicit_manifest_path_single_file(self):
        # explicit_manifest_path triggers legacy single-file load
        self._write_manifest("001-feat", [
            {"data_ref": "header", "disposition": "MATCH"},
            {"data_ref": "footer", "disposition": "SKIP"},
        ])
        refs = _collect_match_refs(
            self._root, "specs/001-feat/design-manifest.json"
        )
        self.assertIn("header", refs)
        self.assertNotIn("footer", refs)

    def test_collect_explicit_manifest_path_missing_returns_empty(self):
        refs = _collect_match_refs(self._root, "specs/nonexistent/design-manifest.json")
        self.assertEqual(refs, set())

    def test_collect_no_manifest_path_globs_all(self):
        # explicit_manifest_path=None → glob
        self._write_manifest("001-a", [{"data_ref": "nav", "disposition": "MATCH"}])
        self._write_manifest("002-b", [{"data_ref": "footer", "disposition": "MATCH"}])
        refs = _collect_match_refs(self._root, None)
        self.assertIn("nav", refs)
        self.assertIn("footer", refs)

    def test_collect_no_manifest_path_no_specs_returns_empty(self):
        # No specs/ dir at all → empty set, Check 5 is no-op
        refs = _collect_match_refs(self._root, None)
        self.assertEqual(refs, set())

    def test_collect_unions_across_multiple_manifests(self):
        # Refs from multiple manifests are unioned
        self._write_manifest("001-a", [
            {"data_ref": "A", "disposition": "MATCH"},
            {"data_ref": "B", "disposition": "SKIP"},
        ])
        self._write_manifest("002-b", [
            {"data_ref": "C", "disposition": "MATCH"},
        ])
        self._write_manifest("003-c", [
            {"data_ref": "A", "disposition": "MATCH"},  # duplicate ref — still one
            {"data_ref": "D", "disposition": "MATCH"},
        ])
        refs = _collect_match_refs(self._root, None)
        self.assertEqual(refs, {"A", "C", "D"})  # B is SKIP, not in refs

    def test_collect_skip_only_manifest_gives_empty(self):
        # All dispositions are SKIP → no MATCH refs
        self._write_manifest("001-a", [
            {"data_ref": "X", "disposition": "SKIP"},
            {"data_ref": "Y", "disposition": "SKIP"},
        ])
        refs = _collect_match_refs(self._root, None)
        self.assertEqual(refs, set())
...
class TestCmdVerifyDesignTokensCLI(unittest.TestCase):
    ...
    def test_enabled_match_literal_exits_2(self):
        # Legacy path: manifest_path in config (back-compat).
        # Uses raw-JSON parse path (_design._schema may be unavailable).
        manifest = {
            "elements": [
                {"data_ref": "sidebar", "disposition": "MATCH"}
            ]
        }
        # Place manifest at a per-feature path (specs/001-feat/design-manifest.json)
        specs_dir = self._root / "specs" / "001-feat"
        specs_dir.mkdir(parents=True)
        manifest_path = specs_dir / "design-manifest.json"
        manifest_path.write_text(json.dumps(manifest))
        # Configure with explicit manifest_path (legacy single-path, relative to root)
        self._config(manifest_path="specs/001-feat/design-manifest.json")
        # Write a CSS file that uses a literal for the MATCH element
        (self._root / "sidebar.css").write_text(textwrap.dedent("""\
            [data-ref="sidebar"] {
              background: #ffffff;
            }
        """))
        code, out, err = self._run()
        # The raw-JSON parser always works; expect exit 2 (MATCH element has literal).
        self.assertEqual(code, 2)
        report = json.loads(out)
        self.assertTrue(len(report["findings"]) >= 1)

    def test_glob_two_manifests_check5_applies(self):
        # New glob path: no manifest_path in config; manifests under specs/A/ + specs/B/.
        # A component with a MATCH data-ref from specs/A/ must trigger Check 5.
        specs_a = self._root / "specs" / "001-feature-a"
        specs_b = self._root / "specs" / "002-feature-b"
        specs_a.mkdir(parents=True)
        specs_b.mkdir(parents=True)
        manifest_a = {
            "elements": [
                {"data_ref": "navbar", "disposition": "MATCH"},
                {"data_ref": "footer", "disposition": "SKIP"},
            ]
        }
        manifest_b = {
        ...
        (specs_a / "design-manifest.json").write_text(json.dumps(manifest_a))
        (specs_b / "design-manifest.json").write_text(json.dumps(manifest_b))
        # Config has only {enabled} — no manifest_path, so glob will be used.
        self._config()
        # Write a CSS file that uses a literal for a MATCH ref from specs/A/
        (self._root / "navbar.css").write_text(textwrap.dedent("""\
            [data-ref="navbar"] {
              background: #ff0000;
            }
        """))
        code, out, err = self._run()
        # Check 5 must fire because "navbar" is in MATCH refs (from glob of specs/A/).
        self.assertEqual(code, 2)
        report = json.loads(out)
        self.assertTrue(len(report["findings"]) >= 1)
        self.assertTrue(any("navbar" in f["summary"] for f in report["findings"]))

    def test_glob_match_ref_from_second_manifest_applies(self):
        # MATCH ref from specs/B/ must be picked up even if the component
        # has nothing to do with specs/A/.
        specs_a = self._root / "specs" / "001-feature-a"
        specs_b = self._root / "specs" / "002-feature-b"
        specs_a.mkdir(parents=True)
        specs_b.mkdir(parents=True)
        (specs_a / "design-manifest.json").write_text(json.dumps({
            "elements": [{"data_ref": "header", "disposition": "SKIP"}]
        }))
        (specs_b / "design-manifest.json").write_text(json.dumps({
            "elements": [{"data_ref": "sidebar", "disposition": "MATCH"}]
        }))
        self._config()
        # CSS file referencing "sidebar" (MATCH from specs/B/) with a hardcoded hex
        (self._root / "sidebar.css").write_text(textwrap.dedent("""\
            [data-ref="sidebar"] {
              background: #0000ff;
            }
        ...
        self.assertEqual(code, 2)
        report = json.loads(out)
        self.assertTrue(any("sidebar" in f["summary"] for f in report["findings"]))

    def test_no_manifest_present_checks1_4_still_run(self):
        # No specs/*/design-manifest.json present, no manifest_path in config.
        # Checks 1-4 must still run; Check 5 is a no-op.
        self._config()
    ...
    def test_no_manifest_present_check5_is_noop(self):
        # No manifests → Check 5 must not fire even if a [data-ref] block has literals.
        # (Because match_refs is empty, no data-ref is MATCH.)
        self._config()
        (self._root / "comp.css").write_text(textwrap.dedent("""\
            [data-ref="anything"] {
              background: #ffffff;
            }
        """))
        code, out, err = self._run()
        # Check 1 fires for the hex literal, but the finding is from Check 1
        # (hardcoded color), not Check 5 (MATCH token binding) — either way exit 2.
        # The point is: no crash and the [data-ref] block itself is not silently
        # promoted to a Check 5 scope error.
        self.assertEqual(code, 2)
        # Check 5 finding text would mention "MATCH" and "token binding" — not present.
        # (Check 1 finding text mentions the hex literal.)
        findings = json.loads(out)["findings"]
        check5_findings = [f for f in findings if "match" in f["summary"].lower()
                           and "token" in f["summary"].lower()]
