...
#### Step 3.3.6: Multi-ecosystem validation gate (BLOCKING for iteration-mode unlock)

**Why this is BLOCKING**: every empirical validation of `/generate-docs` so far has been against testForge20 (Vue 3 / TypeScript / vite / `db-cse-ui-strata/apps/app-web`). That codebase has descriptive naming culture (`QuoteFooter.vue`, `useQuoteState.ts`, etc.) — filename-inferred descriptions land at ~30% useful signal. The architectural claim — "tree + concern overview + glossary is enough for /research regardless of naming culture / build tool / paradigm" — is theory, not validated. The iteration-mode banner removal (multi-package unlock) MUST NOT happen before this gate.

**Why multi-ecosystem (not single cryptic-codebase)**: a single test answers "does it work on ONE other codebase?" — which only weakly bounds the architecture's robustness. A multi-ecosystem suite (3-4 candidates spanning naming culture + build tool + language paradigm) gives empirical evidence that the architecture handles the diversity it'll face when iteration mode is removed.

**Test suite — 3 mandatory candidates + 1 optional**:

1. **Java legacy enterprise** (mandatory). Cryptic-name stress + Maven/Gradle build tool coverage.
   - Pick a legacy enterprise Java codebase with abbreviated module naming (banking/insurance domain heritage, J2EE/EJB-era code, or older Apache Commons subprojects). Modern Spring Boot is acceptable but lower validation value (descriptive naming culture).
   - Tests: extract-package-scripts on Maven (`pom.xml`) or Gradle (`build.gradle` / `build.gradle.kts`); Phase 1 manifest-detection signals for Java; open `--kind` enum (Step 3.3.4) with Java-specific concepts (`interface`, `annotation`, `abstract-class`); deep `com/company/module/` nesting; OOP class hierarchies vs Vue composition.
   - Trivial-leaves coverage: `target/`, `build/`, `.gradle/`, `.idea/`.

2. **Python scientific/numerics** (mandatory). Mathematical-terse naming + pyproject.toml + namespace-package idiom.
   - Candidates: NumPy submodule (`numpy.fft`, `numpy.linalg`), SciPy submodule, or any LAPACK/BLAS-derived library. Modern Django is unacceptable (descriptive naming, low validation value).
   - Tests: extreme cryptic-name stress (`_lapack.py`, `_umath.py`, `core/_methods.py`, single-letter parameters); pyproject.toml manifest (PEP 621 + Poetry); `__init__.py` canonical-aggregator skip-rule; mathematical abbreviations following 50-year-old conventions.
   - Forces architecture to lean on concern overview prose, not tree descriptions — strong validation for Step 3.3.5's "tree is hints" thesis.

3. **Rust crate** (mandatory). Auto-generated names OR older systems naming + Cargo build tool.
   - Candidates: a crate with `bindgen` / `tonic-build` generated output (auto-named binding files); or older systems crate like `nom` (parser combinators with terse function names).
   - Tests: Cargo.toml manifest; `mod.rs` / `lib.rs` canonical-aggregator skip-rule; `--kind trait` / `--kind macro` / `--kind impl` on the open-enum (Step 3.3.4); auto-generated names degrading filename inference; trivial-leaves coverage (`target/`).

4. **C/C++ terse codebase** (optional, high additional value).
   - Candidates: `redis` (C, older, deeply terse), `sqlite`, `lua` itself, or any small game-engine.
   - Tests: header/impl split (`.h` + `.c`) — stress-tests the spec's per-package assumption when there's ONE Makefile at root + scattered files; very cryptic names (`db.c`, `vdbe.c`); Make/CMake build files (currently NOT covered by extract-package-scripts).
   - Optional because C/C++ may surface gaps the helper can't currently address (one Makefile per repo violates the per-package assumption); failure here may indicate a separate helper-side scope rather than an architectural problem.

**What to skip / why** (low validation value):
- Modern Spring Boot Java, modern Django Python, modern React/Next.js — descriptive naming, similar to Vue/TS, redundant with testForge20.
- Massive monorepos (Kubernetes, TensorFlow, Linux kernel) — would dominate test wall-clock; pick smaller representative codebases instead.
- GitHub trending picks bias toward modern descriptively-named projects; for cryptic-name validation, **archive/legacy/scientific** projects are stronger signal even when not trending.

**Procedure per candidate**:

1. Download (clone) the codebase to a sibling directory of the forge repo.
2. Install / setup `/generate-docs` against the candidate's install root.
3. Run `/generate-docs` (still in iteration mode — single-package; pick the most representative package in the candidate's structure).
4. Manually evaluate the resulting concern docs against /research's actual access patterns:
   - **File-location**: "where is `<filename>`?" → tree row mechanical lookup
   - **Symbol-lookup**: "what does `<exported_symbol>` do?" → Public Surface concentrated
   - **Topic-browse**: "tell me about `<concern>`" → concern overview prose
   - **Folder-scoped**: "what's in `<concern>/<subfolder>/`?" → tree section + future glossary (Phase 3.4)
   - **Cross-cutting**: "what features handle `<topic>`?" → topic index + multiple concern reads
5. Score: per-pattern, does the architecture answer the query without requiring /research to read raw source? Mark each pattern pass / partial / fail.
6. Compare across candidates. Identify which patterns degrade where, and whether degradations are architectural (need (B)) or tactical (need wording iteration on the spec).

**Aggregate decision criterion**:

- **PASS** (unlock iteration mode + Step 3.3.4): all 3 mandatory candidates score "pass" or "partial" on every /research access pattern. Specifically: file-location must be PASS on all 3; symbol-lookup and topic-browse must be PASS on at least 2 of 3.
- **CONDITIONAL PASS** (unlock iteration mode but block multi-ecosystem mass-deployment): mandatory candidates pass file-location but degrade on symbol/topic on 1+ candidate. Iterate on the spec until the degradation is closed before going wide.
- **FAIL** (unlock Step 3.3.7 — per-file docs as fallback): file-location fails on any mandatory candidate, OR symbol-lookup fails on 2+ mandatory candidates. Architecture is insufficient for cryptic-name codebases as designed.

**Verify**: empirical assessment writeup added to `GENERATE-DOCS-EXECUTION-LOG.md` with per-candidate access-pattern scores. Aggregate decision recorded. If PASS, iteration-mode banner can be removed in the same iteration as Step 3.3.4. If CONDITIONAL PASS, iterate on spec; re-test; re-evaluate. If FAIL, Step 3.3.7 unblocks.
