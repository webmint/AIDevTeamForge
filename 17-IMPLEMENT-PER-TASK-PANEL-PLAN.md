...
Findings from all 4 reviewers merge on the shared `Critical/High/Medium/Info` scale (anchored to `_audit/findings_schema.py` `SEVERITY_ENUM`, the same enum plan 15 anchors the reviewers' severity vocab to). The panel is "clean" for loop-exit purposes IFF EVERY reviewer returns its own clean verdict (the per-vocab mapping in `## Current state`: code-reviewer `APPROVE` / qa-reviewer `ADEQUATE` / security-reviewer `PASS` / performance-analyst `MEETS TARGETS`). One dirty reviewer keeps the loop going.

### D7 — Reuse the helper-owned ≤3 cap

Keep `REVIEW_LOOP_CAP = 3`, helper-owned; the orchestrator cannot extend it. The panel-merge helper (Phase 3) reuses this SAME constant as its single source of truth (import or mirror — see Phase 3) so the panel loop and the legacy single-reviewer loop cap identically.
...
Only INCOMPATIBLE findings of COMPARABLE severity escalate to Stage A as a conflict decision (the human breaks the tie; the loop then applies the choice and re-reviews to clean). A low-severity finding contradicting a higher-severity one resolves MECHANICALLY by severity order (higher severity wins) — no human needed. The panel-merge helper (Phase 3) computes the `conflicts` list using this comparable-severity rule.
...
- Add a NEW helper module `src/devforge/lib/_implement/_cmds_review_panel.py` exposing a `merge-review-panel` verb. **Decision (stated, not deferred): a NEW module, NOT an extension of `_cmds_review_loop.py`** — so the tested single-reviewer parser stays intact; the two modules SHARE one verdict-vocab table (the per-vocab → clean mapping) as the single source of truth.
- The verb accepts one reviewer-output file PER reviewer keyed by agent name (e.g. repeated `--reviewer <agent-name>:<path>`), parses each with the agent-appropriate verdict vocab (the `## Current state` mapping: code-reviewer `APPROVE` / qa-reviewer `ADEQUATE` / security-reviewer `PASS` / performance-analyst `MEETS TARGETS` = clean), and emits JSON `{clean, escalate, iteration, per_reviewer:[{agent,verdict,clean}], merged_findings:[...], conflicts:[...]}`.
  - `clean` = all reviewers clean (D6).
  - `escalate` = iteration `>= REVIEW_LOOP_CAP` — reuse the constant as ONE source of truth (import from `_cmds_review_loop.py` or mirror with a comment pointing at it; pick one in the python-engineer brief, but keep a single authoritative value per D7).
  - `merged_findings` = findings from all 4 reviewers merged on the unified severity scale (D6), grouped by `(file, line-region)`.
  - `conflicts` = incompatible same-region findings of COMPARABLE severity (D10); lower-severity contradictions of a higher-severity finding are resolved mechanically (higher wins) and do NOT appear in `conflicts`.
- **The verdict-line FINDING regex is now ONE pattern** — `^###\s*verdict\s*:` (reuse the existing `_cmds_review_loop.py` anchor) — because sub-track A made all four verdict lines `### Verdict:` headings. **Precision: sub-track A removed the multi-FORMAT problem, NOT the multi-VOCAB mapping.** The parser still needs a per-reviewer TOKEN → clean map keyed by agent name (code-reviewer `APPROVE`, qa-reviewer `ADEQUATE`, security-reviewer `PASS`, performance-analyst `MEETS TARGETS` = clean); one regex locates the verdict line, the keyed vocab table classifies its token. Tests round-trip the POST-normalization `### Verdict:` line for all four reviewers with their distinct tokens (NOT the old `- Verdict:` bullet or bare `Verdict:` shapes — those no longer exist after sub-track A).
...
- **Sub-track B:** `merge-review-panel` parses the verdict line with ONE regex (`^###\s*verdict\s*:`) and classifies the token via the per-reviewer vocab map (code-reviewer `APPROVE` / qa-reviewer `ADEQUATE` / security-reviewer `PASS` / performance-analyst `MEETS TARGETS` = clean), aggregates `clean` (all-clean per D6) + `escalate` (one source-of-truth cap per D7), merges findings on unified severity, and detects comparable-severity conflicts (D10); the new test module round-trips the post-normalization `### Verdict:` line for all four reviewers with their distinct tokens (NOT the old bullet/bare shapes) and is green; the single-reviewer `_cmds_review_loop.py` parser is untouched and still green.
...
- Rewrite `src/commands/implement/main.md` PHASE 6: fan out the 4 reviewers IN PARALLEL via the Task tool (one turn, four Task calls) over the frozen `touched_files`; write each return to a scratch file; call `merge-review-panel` with one `--reviewer <agent>:<path>` per reviewer; branch on the aggregate:
  - **clean** ⇒ proceed to the forcing-functions gate (`run-forcing-functions-gate`, unchanged) then PHASE 7.
  - **not-clean + not-escalate** ⇒ ONE implementing-agent repair leg addressing ALL merged non-conflicting findings; conflicts + judgment calls recorded as decision items (the existing mechanical-vs-judgment classification still applies); then re-run the FULL panel (D9) at iteration `N + 1`.
...
- Update `src/commands/implement/references/review-loop.md` to describe the PANEL loop: the per-vocab → clean mapping (all 4), the all-clean rule (D6), the conflict decision-item shape (D10), and the full-panel-each-round rule (D9).
- Update `src/commands/implement/references/agent-brief.md` so the PHASE 6 re-dispatch brief carries the MERGED panel findings (not just code-reviewer's) into the repair leg.
...
DoD: PHASE 6 fans out 4 read-only reviewers in parallel, merges via `merge-review-panel`, repairs once per round addressing all merged findings, re-reviews the full panel each round (D9), and is capped (D7); the forcing-functions gate is unchanged and still after the loop; `review-loop.md` + `agent-brief.md` reconciled to the panel; the rewritten `review-loop.md` panel description must NOT introduce or carry over an `accept anyway` option (Phase 5 removes it from the could-not-converge path; Phase 4 must not re-add it); parallel-dispatch verified via `claude-code-guide` before writing.
...
#   - merge-review-panel JSON shows per_reviewer for all 4 + merged_findings.
...
- **Phase 3 now has two ordered sub-tracks** (mirroring plan-15's Phase-4 sub-track pattern): **sub-track A (markdown loop, lands FIRST)** normalizes `security-reviewer.md` + `performance-analyst.md` verdict lines to `### Verdict:` headings via `instruction-author` → `instruction-reviewer` (the D8 carve-out; regenerate emitted agents + run `tests/scripts/test_generate_agents.py`); **sub-track B (python loop, lands AFTER A)** builds the panel-merge helper. The panel-merge helper is a NEW module `_cmds_review_panel.py` (verb `merge-review-panel`) — NOT an extension of `_cmds_review_loop.py`; the single-reviewer parser stays intact; both share one verdict-vocab table and one cap constant. Because sub-track A unified the verdict-line FORMAT, sub-track B's verdict-line FINDING regex is ONE pattern (`^###\s*verdict\s*:`, reusing the `_cmds_review_loop.py` anchor); the per-reviewer TOKEN→clean VOCAB map (keyed by agent name) is still needed — A removed the multi-format problem, not the multi-vocab one.
