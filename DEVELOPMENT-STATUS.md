...
- `breakdown.md` — **Redesigned (`09-BREAKDOWN-COMMAND-REDESIGN-PLAN.md`, 2026-05-25)** — aligned with the redesigned chain. Consumes the approved plan's sibling `plan-handoff.json` (helper `breakdown_helper read-plan-handoff`); 11 phases mirroring `/plan` (plan resolution → upstream-handoff consume → status flip → context → deep file analysis → REQUIRED Findings-from-Plan enumeration → mandatory+scoped architect decomposition consult → write tasks → contract-chain + AC-coverage forcing-function gates → hard-gate approval → finalize). Emits human `tasks/NNN-*.md` (storage-rules format) + a structured `breakdown-handoff.json` (schema `_breakdown/handoff_schema.py`, `handoff_kind="breakdown"`) — producer side of the breakdown→`/execute-task` handoff (consumer conforms when built). Agent-assignment table **inlined** into the command (sole owner — `/fix`+`/refactor` slated for removal). Helper `breakdown_helper` (13 verbs, 232 tests) owns all structural emission
...
- `_agent-assignment.md` — Shared file-layer→agent mapping table, referenced by fix and refactor (`/breakdown` now inlines its own copy per `09-BREAKDOWN-COMMAND-REDESIGN-PLAN.md` OQ-3)
