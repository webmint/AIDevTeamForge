...
**Status**: All 5 patches applied + tests green (231/231 in `tests/lib/test_research_helper.py`). F2 closed by Patch 2 (layer-boundary stopping rule + check 8b cross-layer gate + `record-fix-path-helper --file-line`). Patch 3 closed Gap 3 (`set-scope` evidence gate on narrow framing). Patch 4 closed Gap 4 (`set-recommended-approach` single-layer gate with `--single-layer-justification` + `--cites` + check 13 verify-time mirror + 8b-precedence suppression). Patch 5 closed Gap 5 (`record-fix-path-helper` anchor gate w/ ±5 line tolerance + sticky-reject log against post-hoc-anchor adversarial path + check 14 verify-time mirror). **Empirical verification pending**: re-dispatch /research on testForge20 duplicate-options topic + diff against `testForge20/tmp/2026-05-16-restriction-on-adding-the.md`.
**Date**: 2026-05-16
