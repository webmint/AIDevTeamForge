...
**Status**: Phases 0–7 DONE 2026-08-05/06 — **D1–D9 RATIFIED by the maintainer 2026-08-05** (interactive session) + **D10 ratified in-session 2026-08-06**. Commits: `4bd613c` substrate, `52a417c` producer helpers, `b42e75a` producer specs, `43bfbf2` + `086cf01` consumer (atomic unit CLOSED), `0e25e3a` downstream + D10 record, `75961dd` docs/prefix reconcile. **Phase 7 sweep results (2026-08-06):** full suite 9811 passed / 14 skipped / 133 subtests, zero failures (`tests/lib/_spec_check` excluded — its z3 dependency is absent from `.venv-test`, a pre-existing env gap unrelated to this plan); `research/` + `discover/` greps over `src/` + `scripts/` fully classified — every hit is a `${TMPDIR:-/tmp}/forge-research` scratch path, an annotated legacy prefix-list entry, or a retired-layout back-reference/comment; bare-`handoff.json` conflation grep: 0 hits; `verify-agent-reachability` PASS; fresh install ride PASS (all 17 commands emit, `allocate-feature-dir`/`render-branch-command`/`set-spec-number` executable in the installed tree, `{{` hits are the normal pre-`/configure` placeholders). **Remaining: Phase 8 ONLY — testForge20 consumer e2e (user-driven HARD GATE; six scenarios in the Phase 8 section).**
