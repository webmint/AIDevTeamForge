...
```
{{output.sigil}}setup-wizard → {{output.sigil}}constitute → {{output.sigil}}onboard → {{output.sigil}}research → {{output.sigil}}specify → {{output.sigil}}plan → {{output.sigil}}breakdown → {{output.sigil}}execute-task → {{output.sigil}}review → {{output.sigil}}verify → {{output.sigil}}summarize → {{output.sigil}}finalize
   (once)         (once)       (once)    (optional)  (per feat)  (per feat) (per feat)   (per task)    (per feat) (per feat) (per feat)  (per feat)
```

- `{{output.sigil}}specify "feature"` — Create spec with acceptance criteria → `specs/NNN-name/spec.md`
- `{{output.sigil}}plan` — Technical plan from approved spec → `specs/NNN-name/plan.md`
- `{{output.sigil}}breakdown` — Atomic tasks with dependencies → `specs/NNN-name/tasks/`
- `{{output.sigil}}execute-task N` — Implement one task with assigned agent
- `{{output.sigil}}review` — Security + performance + test review → findings report
- `{{output.sigil}}verify` — Check acceptance criteria against spec
- `{{output.sigil}}finalize` — Squash WIP commits, generate docs, clean commit

Standalone (no spec required):
- `{{output.sigil}}fix "bug"` — Localized bug fix (1-5 files)
- `{{output.sigil}}refactor path "goal"` — Behavior-preserving restructuring (1-5 files)
- `{{output.sigil}}security [target]` — On-demand security review
...
#### `{{output.sigil}}breakdown [spec-file]`
...
#### `{{output.sigil}}execute-task [number]`
...
5. Memory update
WIP commits accumulate across tasks and are squashed by `{{output.sigil}}finalize` when the feature is approved.

#### `{{output.sigil}}review [spec-file]`
Launches specialist review agents (security, performance, test assessment) on completed feature code. Produces a structured review report saved to `specs/[feature]/review.md` for `{{output.sigil}}verify` to incorporate into its verdict. Does not render a verdict — findings only.

#### `{{output.sigil}}verify [spec-file]`
Verifies all completed tasks against the spec's acceptance criteria. When AC verification is enabled (`AC_VERIFICATION` in project-config.json), launches the **ac-verifier** agent to test acceptance criteria against the running app via Chrome DevTools MCP and/or API calls — falls back to code reading when MCP is not available. Incorporates `{{output.sigil}}review` findings if available (warns if missing). Performs cross-task integration check (not full code review — that was done per-task). Updates memory with lessons learned.

#### `{{output.sigil}}summarize [spec-file]`
Generates a concise, PR-ready summary of a completed feature. Reads spec, plan, tasks, and git history. Saves to `specs/[feature]/summary.md`. Run after `{{output.sigil}}verify` approves, before `{{output.sigil}}finalize`.

#### `{{output.sigil}}finalize [spec-file]`
Generates feature-level documentation via tech-writer, then squashes all WIP commits into a single clean feature commit. Gate-checked: spec must be Complete (set by `{{output.sigil}}verify`). The last step before creating a PR.

#### `{{output.sigil}}constitute`
...
#### `{{output.sigil}}onboard`
One-time deep codebase scan for existing projects. Uses the tech-writer agent to generate comprehensive documentation in `docs/` — the knowledge base for all agents. Run once after `{{output.sigil}}constitute`.

#### `{{output.sigil}}fix "bug description"`
...
If the bug grows beyond 5 files, recommends escalating to `{{output.sigil}}specify`.

#### `{{output.sigil}}refactor path/to/file.ts "goal"`
...
If the refactoring grows beyond 5 files, recommends escalating to `{{output.sigil}}specify`.

#### `{{output.sigil}}security [file|dir|--full]`
...
#### `{{output.sigil}}audit [--full | --uncommitted | path]`
...
Agent selection is automatic in `{{output.sigil}}execute-task` based on the task's assigned agent.

## Enforced Quality Gates

### Hard Gates (block until approved)
- Spec approval → before `{{output.sigil}}plan` can run
- Plan approval → before `{{output.sigil}}breakdown` can run
- Task breakdown approval → before `{{output.sigil}}execute-task` can start
- Acceptance criteria → verified in `{{output.sigil}}verify`

### Verification (explicit, scope-aware — no runtime hooks)

Verification runs at task boundaries (end of `{{output.sigil}}execute-task`, `{{output.sigil}}fix`, `{{output.sigil}}refactor`, etc.), not after every file edit. Both Claude and Codex behave identically — no runtime hooks, no auto-execution after Edit/Write. Verification is **scope-aware**: the phase reads `PACKAGE_STACKS` (see `## Packages` above) to determine which type-check / lint / build commands apply to each file touched during the task.
...
Full specification in `{{output.sigil}}execute-task`.
...
13. **Session state** — after each `{{output.sigil}}execute-task`, overwrite `.devforge/session-state.md` with a fixed-size snapshot of current progress. At session start, read it first if it exists.
14. **Crash recovery** — `{{output.sigil}}execute-task` writes a WIP marker (`.devforge/wip.md`) before execution and creates git checkpoints at each phase. If interrupted, the next run detects it and offers resume/rollback/skip options.
...
```
research/
  YYYY-MM-DD-topic-slug.md        # Research reports ({{output.sigil}}research) — exploratory, pre-spec

specs/
  001-feature-name/            # Numbered feature directories
    spec.md                    # {{output.sigil}}specify output
    plan.md                    # {{output.sigil}}plan output
    research.md                # {{output.sigil}}plan research (optional)
    data-model.md              # {{output.sigil}}plan data model (optional)
    contracts.md               # {{output.sigil}}plan API contracts (optional)
    tasks/                     # {{output.sigil}}breakdown output
...
- **Updated automatically** by `{{output.sigil}}execute-task` (Phase 7)

If context is compacted or a new session starts, session-state.md ensures the next `{{output.sigil}}execute-task` can bootstrap without re-discovering state.

### Crash Recovery

If a task execution is interrupted (power loss, terminal crash, network drop), the next `{{output.sigil}}execute-task` will detect the interrupted state via `.devforge/wip.md` and offer recovery options: resume from where it stopped, rollback and retry, rollback and skip, or keep changes for manual handling. The WIP marker includes a `Command` field identifying which command (`{{output.sigil}}execute-task`, `{{output.sigil}}fix`, or `{{output.sigil}}refactor`) was interrupted — if you run a different command, it will detect the mismatch and ask you to resolve the previous session first. Git checkpoint commits (`[WIP]` prefix) preserve partial work and are squashed into a clean feature commit by `{{output.sigil}}finalize` when the feature is approved.
