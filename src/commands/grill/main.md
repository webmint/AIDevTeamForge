---
name: grill
description: Standalone adversarial design-grill — the design-level mirror of `/devforge:review`. Runs by invocation between `/devforge:plan` and `/devforge:breakdown` to attack the FINISHED design (`plan.md` + its `spec.md`) before `/devforge:breakdown` spends effort decomposing it. Dispatches the `devils-advocate` adversary (which resolves a three-ring codebase blast radius and self-gated web-verification) TWICE and unions the two passes' findings, cross-examines every attack in that union with a non-author refutation pass (architect excluded), writes `<feature_dir>/grill.md`, and recommends a 4-way disposition (PROCEED / REVISE-PLAN / RE-ENTER-UPSTREAM / KILL). A run where nothing survived cross-examination presents the result and ends without asking a question. Human-typed only — `/devforge:breakdown` requires that it RAN, never that its disposition binds.
