...
10. **Iterate per-concern slot-fill SEQUENTIALLY in the orchestrator's own context — one concern per loop iteration, complete its slot-fill before starting the next.** Do NOT dispatch subagents and do NOT use parallel iteration. For each `(package_path, concern_name, subfolder)` triple from the step 8 worklist, execute the per-concern slot-fill workflow inline (register concern, render concern skeleton, read source under `subfolder`, invoke concern-tier setters with `extract-snippet` precedence per step 4's directive, run `validate-concern` with capped retries, render concern doc).
    ...
    **Tree composition (single dispatch per concern — Part D, post-2026-05-07 revert).** Compose the `set-concern-tree --text` value in ONE dispatch — either via the `tree-annotator` Task subagent or inline in the orchestrator's own context. The subagent receives the concern subfolder's index.json file list + sibling-concern overviews + the description-rule above + the trivial-leaves rule above; it returns the full ASCII tree text with inline `# <description>` comments per non-trivial entry. Orchestrator passes that text to `set-concern-tree --text` in a single call.

        - Single dispatch per concern caps cost: testForge20-scale package = ~7 dispatches per run (one per concern), not ~600 (one per source file). Per-file recall is preserved via the inline tree descriptions — `# <description>` per file in the tree section is the file-level retrieval index for /research, no separate per-file `.md` document required.

        - Annotation discipline rules (banned phrases, specificity test, vs-siblings differentiation, evidence-first ordering, negative-space test) apply to the WHOLE tree composition: every entry's description follows them. The tree-annotator agent enforces these in its prompt; orchestrator-direct composition follows the same rules inline.

        - Per-md helpers (`render-file-skeletons`, `write-file-doc`, `validate-file-doc`, `verify-file-docs`) remain available in the helper CLI as dormant primitives for future revival (codegraph-augmented batch dispatch, per Part C planning) but are NOT invoked from the active /generate-docs flow. The `_check_file_docs_complete` rule in `validate-concern` is also dormant — `validate-concern`'s active rule chain enforces required-fields, public-surface, codeblocks, enums, no-todos, optional-render only.
    ...
    **This per-concern iteration does NOT:**
    ...
    - **Invoke package-tier or architecture-tier setters.** Per-concern scope is concern-tier only: `add-concern`, `render-concern-skeleton`, `set-concern-overview`, `set-concern-tree`, `add-concern-export`, `add-concern-type`, `add-concern-dep`, `add-concern-hazard`, `set-concern-usage-example`, `validate-concern`, `render-concern-doc`, plus the read-only `status` subcommand (used in resume mode to inspect current concern state before each conditional setter) and `extract-snippet` (used for verbatim snippet extraction). Per-md helpers (`render-file-skeletons`, `write-file-doc`, `validate-file-doc`, `verify-file-docs`) are dormant per Part D — available in the helper CLI but not invoked from this active flow. Package-tier setters are out of scope and MUST NEVER be invoked from per-concern iteration: `add-package`, `set-package-overview`, `set-package-tree`, `set-package-language`, `set-package-framework`, `set-package-build-tool`, `add-package-script`, `add-package-export`, `add-package-dep`, `add-package-hazard`, `set-package-usage-example`, `set-package-consumer-pattern`. Architecture-tier setters are likewise out of scope.
