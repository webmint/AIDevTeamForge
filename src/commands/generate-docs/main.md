...
10. **Iterate per-concern slot-fill SEQUENTIALLY in the orchestrator's own context — one concern per loop iteration, complete its slot-fill before starting the next.** Do NOT dispatch subagents and do NOT use parallel iteration. For each `(package_path, concern_name, subfolder)` triple from the step 8 worklist, execute the per-concern slot-fill workflow inline (register concern, render concern skeleton, read source under `subfolder`, invoke concern-tier setters with `extract-snippet` precedence per step 4's directive, run `validate-concern` with capped retries, render concern doc).
    ...
    **Per-md fill loop (runs after `set-concern-tree`, before `validate-concern`).** For the concern's source subfolder, fill each `.md` skeleton with structured front-matter via the per-md fill loop:

        1. **Render skeletons.** Invoke `.devforge/lib/generate_docs_helper render-file-skeletons --package <package_path> --concern <concern_name>`. The helper walks `index.json` files filtered to `src/<concern_name>/`, applies the trivial-leaf skip rule (`_path_contains_trivial_dir` — excludes paths through `node_modules`, `__pycache__`, `dist`, etc.), and creates one empty `.md` per surviving source file at `<project_root>/docs/<package_path>/<concern_name>/<rel-suffix>.md`. Re-runs are idempotent — pre-existing filled mds are not overwritten. Exit 0 → continue. Exit 2 → validation failure (package or concern not registered, or index.json missing or package not in index); capture stderr, record in concern errors, abort the concern.

        2. **Cost-estimate user gate.** After skeletons render, count empty `.md` files under `<project_root>/docs/<package_path>/<concern_name>/` (size == 0 bytes). Compute estimate:
           - Per-md cost: ~$0.01–$0.03 Haiku (input + output tokens)
           - Per-md wall-clock: 6–15 seconds (Haiku dispatch + helper validate cycle)
           - Per concern budget: 100 mds ≈ $1–$3, 5–25 minutes

           Print the estimate to the user as plain prose (skeleton count + dollar range + wall-clock range). Then ask via AskUserQuestion (single-line prompt: `Proceed with per-md fill loop for <concern_name>? (yes/no)`). On "no", record `errors: ["user declined per-md fill at cost-estimate gate"]` in the concern outcome and proceed to the next concern.

        3. **Build sibling context map.** For each empty `.md` under the concern's docs dir, enumerate peer `.md` files in the same immediate parent directory, paired with their labels parsed from their front-matter (if already filled). Empty siblings have no label and are skipped from the context. Build a dict `{md_path: [(sibling_path, sibling_label), ...]}` for use in the dispatch loop.

        4. **Per-md dispatch loop.** For each empty `.md` (size == 0 bytes) in the concern's docs dir:

            a. **Resolve `source_path`.** From `md_path`, strip the `<project_root>/docs/<package_path>/<concern_name>/` prefix and append the source-file path: `source_path = <package_path>/src/<concern_name>/<rel-suffix-without-.md>`. Strip the trailing `.md` from the md_path filename to recover the original source filename.

            b. **Dispatch the `tree-annotator` Task subagent** with input context `(target_md_path=<md_path>, source_path=<source_path>, concern=<concern_name>, siblings=<sibling list from step 3>, previous_attempt_feedback=<unset on first attempt>)`. The subagent invokes `write-file-doc` directly via Bash; on its Bash call exit 0 the agent emits the assistant line `wrote: <md_path>` as its final output. Detection by orchestrator: read the agent's final assistant line — `wrote: <md_path>` indicates success; verbatim stderr indicates the agent's Bash call failed and the orchestrator must retry.

            c. **Invoke `validate-file-doc`**: `.devforge/lib/generate_docs_helper validate-file-doc --md-path <md_path>`. Exit 0 → md accepted; move to the next md. Exit 2 / 3 / 4 / 5 / 6 → capture stderr and proceed to step 4d.

            d. **On non-zero exit, retry with feedback.** Copy the captured stderr VERBATIM into your next `tree-annotator` dispatch's `previous_attempt_feedback` field as a fenced code block — do not summarize or paraphrase. Re-dispatch `tree-annotator` (the agent's own `write-file-doc` call OVERWRITES the previous md attempt — no manual cleanup needed). Re-invoke `validate-file-doc`. Cap at 3 attempts total per md (initial dispatch + 2 retries).

            e. **After 3 failed attempts, escalate to Sonnet.** Dispatch `tree-annotator` ONE final time using the Task tool's `model: "sonnet"` override (this controls the model Claude Code runs the agent on). Include `model_version: sonnet` in the dispatch instructions so the agent passes `--model-version sonnet` to `write-file-doc` (the Task tool's `model:` override does NOT auto-propagate to the helper's CLI arg — these are independent). Pass the most recent stderr as `previous_attempt_feedback`. Re-run `validate-file-doc`. Single attempt only — no further retries at the Sonnet tier.

            f. **If escalation also fails, fall back to `ambiguous`.** The orchestrator does NOT pass `--confidence ambiguous` directly to `write-file-doc` — instead, dispatch `tree-annotator` one more time with an explicit instruction in `previous_attempt_feedback`: "All retries failed; use confidence=ambiguous with the cite range from your last attempt and a label that names the rejection reason verbatim (e.g., `cite-resolution failed for src/...`)." On still-failing `validate-file-doc`, record the entry in the concern's outcome `errors` array as `{"md_path": "<md_path>", "reason": "<final stderr>"}` and proceed to the next md. Do NOT block the whole concern on a single md's fill failure.

        5. **Post-batch quality gate.** Invoke `.devforge/lib/generate_docs_helper verify-file-docs --package <package_path> --concern <concern_name>` after the per-md dispatch loop completes. Three exit codes:
           - Exit 0 — all gates pass (banned-phrase = 0, ambiguous rate ≤ 10%, cross-concern duplicate rate ≤ 5%, vacuous_pass: not a vacuous pass — if `directory_tree` is set in state, at least one filled md exists; if tree unset, total may be 0). Proceed to step 6.
           - Exit 2 — at least one gate failed. Capture the JSON report from stdout and the failing-gate lines from stderr; copy both VERBATIM into your next user-facing message as a fenced code block (do not summarize or paraphrase). Halt the concern with the report shown; the user decides whether to re-run the per-md fill loop, edit thresholds, or accept the failure. Do NOT silently skip.
           - Exit 5 — state error (package or concern not registered, front-matter parse error in any md, or state-corrupt confidence value). Capture stderr verbatim, record `{"reason": "<stderr>"}` in the concern's outcome `errors` array, and abort the concern.

        6. **Post-loop validation.** After all empty mds in the concern's docs dir have been processed (filled, ambiguous-tagged, or recorded as errors), proceed to the per-concern workflow's `validate-concern` invocation (still part of step 10 per the per-concern intro at the start of this step). `validate-concern` enforces the `file-docs-incomplete` rule (Step B.2): walks the docs dir, asserts every expected md is filled to >50 bytes. Any unfilled md surfaces as a `file-docs-incomplete` validation error there. This filesystem walk is the structural gate; the orchestrator cannot bypass it because `validate-concern` is a mandatory pre-render gate.

        **Verbatim stderr discipline (mandatory).** Step 4d says "copy VERBATIM... do not summarize or paraphrase" because the validator's stderr is the only signal the annotator has about why its prior attempt was rejected. Pass the captured stderr to the next dispatch as a fenced code block: `previous_attempt_feedback:` followed by a triple-backtick fence containing the literal stderr text. Paraphrased feedback loses the exact banned-phrase token, the exact cite-line numbers, or the exact sibling label that collided — all of which are load-bearing for the annotator's next attempt.
    ...
    **This per-concern iteration does NOT:**
    ...
    - **Invoke package-tier or architecture-tier setters.** Per-concern scope is concern-tier only: `add-concern`, `render-concern-skeleton`, `set-concern-overview`, `set-concern-tree`, `add-concern-export`, `add-concern-type`, `add-concern-dep`, `add-concern-hazard`, `set-concern-usage-example`, `validate-concern`, `render-concern-doc`, plus the per-md fill helpers `render-file-skeletons`, `write-file-doc`, `validate-file-doc` (invoked by the per-md fill loop), plus the read-only `status` subcommand (used in resume mode to inspect current concern state before each conditional setter) and `extract-snippet` (used for verbatim snippet extraction). Package-tier setters are out of scope and MUST NEVER be invoked from per-concern iteration: `add-package`, `set-package-overview`, `set-package-tree`, `set-package-language`, `set-package-framework`, `set-package-build-tool`, `add-package-script`, `add-package-export`, `add-package-dep`, `add-package-hazard`, `set-package-usage-example`, `set-package-consumer-pattern`. Architecture-tier setters are likewise out of scope.
