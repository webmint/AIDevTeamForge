...
**`/devforge:review` produces FINDINGS ONLY — it does NOT render a verdict.** The verdict is `/devforge:verify`'s job: `/devforge:verify` consumes `specs/[feature]/review.md` (folding its findings into the verdict, and warning if it is missing), and `/devforge:audit` reads recent `specs/*/review.md` files for its recurring-issue scan. Do not add a pass/fail line, an approval line, or a "ready to ship" judgment.

Usage: `/devforge:review` (auto-resolve the most-recently-modified `specs/NNN-*` feature) · `/devforge:review specs/001-auth` or `/devforge:review specs/001-auth/spec.md` (an explicit feature dir or a spec file inside it).
...
- `specs/[feature]/review.md` — the rendered feature-review report. Produced by the helper's `render-report` verb in PHASE 4; FINDINGS ONLY (no verdict). Idempotent: re-running `/devforge:review` on the same feature OVERWRITES `review.md` (the helper does an atomic write). `/devforge:verify` consumes it next.
...
- When `$ARGUMENTS` is empty, auto-resolve the most-recently-modified `specs/NNN-*` directory (the feature most likely just finished `/devforge:implement`).

If no `specs/NNN-*` directory exists, tell the user there is no feature to review (run `/devforge:specify` → `/devforge:plan` → `/devforge:breakdown` → `/devforge:implement` first) and end the turn. Carry the resolved feature dir forward as `<feature>` — every subsequent `--feature` / `--feature-dir` flag takes it.
...
Compute the assembled-feature diff — the union of every change the feature made, across all the WIP commits `/devforge:implement` accumulated (squashed only by `/devforge:finalize`, which has not run yet). This is exactly the cross-task surface the per-task panel never saw. Substitute `<feature>` and the `source_root` from `$WORKDIR/preflight.json` (PHASE 0) — `source_root` is `"."` for a standalone install and the inner project subdir path in wrapper mode; pass it verbatim to `--source-root`:
...
**Known pitfalls from prior sessions (prepend, when there are any).** Alongside the fields 0.1 names, the `preflight` stdout re-captured at `$WORKDIR/preflight.json` carries `memory_present` (bool) and `memory_excerpt` (the first 40 lines of `.devforge/memory.md`, the project's persistent cross-session lessons file). Read `memory_excerpt` from that file and pick out the entries bearing on this feature's changed files (`$WORKDIR/scope.json`). When there are any, PREPEND them to the rendered brief as a short, clearly-labelled block before passing it as the Task prompt — prepend, never append, so the closing Bash-write reminder stays the brief's last instruction. Emergent cross-task review is the most judgment-heavy step in the pipeline, and a recorded past failure in the same area is the cheapest available direction for that judgment: it tells the finder where to look hardest. **Honesty bound.** A memory entry is an UNVERIFIED prior-session assertion, not evidence — the code it describes may have changed since it was written, or the entry may have been wrong then. It directs attention; it never substitutes for reading the assembled diff, and it is never a finding's Evidence: `validate-findings` (2.4) grounds every finding against a verbatim quote from the real cross-task code, so a finding supported only by a memory entry is discarded there. When `memory_present` is false, or the excerpt holds only the shipped stub's headings with no entries under them, prepend nothing and say nothing to the user about memory — a project with no recorded lessons is the ordinary state of a fresh install, not a fault to remedy.
...
Substitute `<feature>` with the resolved feature dir and `<NNN>-<slug>` with the feature id. `commit-artifacts` stages ONLY the named paths and makes a `[WIP] review: <NNN>-<slug>` commit in the INSTALL repo (never the wrapper-mode source/product repo). It is FAIL-SOFT: a git staging or commit failure warns on stderr and exits 1 (non-fatal — the report is already written, so note the warning and CONTINUE; do NOT end the turn); "nothing to commit" (paths already staged or absent) exits 0 silently as a benign no-op. The `[WIP]` commit reads only the named `specs/[feature]/` paths, NOT `$WORKDIR`, so it is safe to run before the scratch cleanup. The `[WIP]` commit folds into `/devforge:finalize`'s squash, leaving the final PR unchanged.

Then point the user to the next step: tell them `specs/[feature]/review.md` was written (findings only) and WIP-committed, and the next command is `/devforge:verify` — which consumes `review.md` and folds its findings into the acceptance-criteria verdict.
...
1. **Findings only, NO verdict** — `/devforge:review` never renders a pass/fail, an approval, or a "ready to ship" line. The verdict is `/devforge:verify`'s job; `/devforge:review` produces `specs/[feature]/review.md` and `/devforge:verify` consumes it.
2. **`review.md` is a pipeline artifact** — it feeds `/devforge:verify` (which folds its findings into the verdict and warns if it is missing) and `/devforge:audit`'s recurring-issue scan (which globs recent `specs/*/review.md` files). It must stay a markdown file at `specs/[feature]/review.md`.
...
4. **Idempotent** — re-running `/devforge:review` on the same feature OVERWRITES `specs/[feature]/review.md` (the helper's atomic write); there is no per-day suffixing — one feature, one review report.
