...
/verify confirms each carried row's anchor token is absent from the post-change code (plan 71 Phase 4).
...
**Property coverage** — every pure-builder target `/plan` declared is covered by a dedicated property-test task:
...
- Exit 0 (`property-coverage: skip (no declared pure-builder targets)`) → `/plan` declared no pure-builder targets (the handoff exists but the key is absent or the list is empty); this is not a property-lane feature. No action.
...
**Dead-code coverage** — every change-induced dead-code row `/plan` declared is folded into exactly one owning task:
...
- Exit 0 (`dead-code-coverage: skip (no declared dead-code rows)`) → `/plan` declared no change-induced dead code (the handoff exists but `breakdown_seeds.dead_code_rows` is absent or empty); this is not a dead-code-lane feature. No action.
...
The `**Property coverage**:` line is likewise CONDITIONAL — include it ONLY when `/plan` declared pure-builder targets (i.e. the PHASE 3.5 property-coverage gate returned `ok`, not `skip`). When there are no pure-builder targets, OMIT the line entirely.
