...
- **Header fields**: Agent (from the Agent Assignment table), Depends on, Blocks, Spec criteria (`AC-N`), Review checkpoint (Yes/No — see below), Context docs (see below), plus Property targets on dedicated property-test tasks only (see the Property-test tasks subsection below).
...
- Non-zero exit → the helper could not write or validate the handoff. `finalize-handoff` runs the roster check AND the design-manifest assertion AND the property-coverage check (when a sibling `plan-handoff.json` declares pure-builder targets) internally as backstops, so capture BOTH stdout and stderr and branch on their content (the design-manifest and property-coverage backstops never emit to stderr from `finalize-handoff` — their failures surface as `## Design manifest findings` / `## Property coverage findings` blocks on stdout):
