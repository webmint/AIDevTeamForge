...
- `<feature_dir>/summary.md` — the rendered feature summary: a conditional `**Run by**:` provenance line in the header, then up to eight sections (What was built / Review guide / Changes / Files changed / Key decisions / Deviations / Acceptance criteria / Not verified). Three of the eight are conditional — Review guide, Deviations and Not verified are each omitted entirely when empty — so a feature with nothing to orient by, no deviations and nothing unverified renders the other five. Composed INLINE by the orchestrator in PHASE 3 and written with the Write tool in PHASE 4. Idempotent: re-running `/devforge:summarize` on the same feature OVERWRITES `summary.md`.
...
- `$WORKDIR/verification.json` — the `read-verification` stdout (`ac_list`, `verdict`, `path`, `e2e_status`, `has_scope_creep_advisory`, `has_leftover_artifacts_advisory` — six keys). Written in PHASE 2, read by the orchestrator for the Acceptance-criteria section (the AUTHORITATIVE AC status), the referenced verdict, and the Not-verified section (the last three keys). There is deliberately NO `reasons` key: the helper extracts `e2e_status` from `verification.md`'s Reasons bullets itself, so the orchestrator is never handed a prose list to mine.
- `$WORKDIR/notes.json` — the `parse-completion-notes` stdout (a JSON array, one dict per task: `completed_at`, `files_changed`, `expects_met`, `produces_met`, `notes`, `has_unverified`, `has_notes`, `task_file`). Written in PHASE 2, read by the orchestrator for the Changes + Deviations sections, and — via `has_unverified` — for the Not-verified section.
...
- `references/summary-format.md` — the `summary.md` skeleton the orchestrator composes (orientation for PHASE 3; documents the eight sections, the omit-when-empty rule for each of the three conditional ones — Review guide, Deviations, Not verified — and that AC status comes from `verification.md`. The synthesis is inline prose — there is no `write-summary` helper verb).
...
`read-verification` parses `/devforge:verify`'s `<feature_dir>/verification.md` into `ac_list` (one dict per AC with `id`, `status`, `evidence` — the status is the AUTHORITATIVE per-AC result `/devforge:verify` already proved), `verdict` (`APPROVED` / `NEEDS WORK` / `REJECTED`, or `""` when none was found), and `path` (the file read). **The AC status comes from `verification.md`'s table, NOT re-derived from the spec** (D3) — `/devforge:verify` already proved each AC PASS/FAIL/PARTIAL, and `/devforge:summarize` reports that result verbatim. The `verdict` is REFERENCED in the summary (it is `/devforge:verify`'s verdict — `/devforge:summarize` does not compute it).

The same stdout carries three more fields the helper parsed out of the SAME report, and the Not-verified section (PHASE 3) is their only consumer:

- `e2e_status` — the status token of the e2e run `/devforge:verify` recorded, or `null` when it recorded none. The helper lifts it from the Reasons bullets under `## Verdict`; the orchestrator receives the token and never reads that prose. **Render it only when it is NON-NULL and not `e2e-clean`** (see PHASE 3). Two values never appear here: an install with no e2e suite configured, and a run whose gate returned `off`, both produce NO bullet for the helper to parse and so arrive as `null` — do not write a comparison against `"off"`, and do not treat `null` as a status worth naming.
- `has_scope_creep_advisory` and `has_leftover_artifacts_advisory` — bools, true when `/devforge:verify`'s `## Code Quality` section flagged that advisory. Both are advisory conditions `/devforge:verify` flagged without blocking the verdict, and the summary names them when present.

There is no `reasons` key and none is to be added: the helper owns the extraction so this command reports a recorded status rather than interpreting a list of sentences.
...
Compose the summary INLINE from the scratch inputs — `$WORKDIR/changes.json`, `$WORKDIR/verification.json`, `$WORKDIR/notes.json`, `$WORKDIR/decisions.json` — plus one direct read of the task files, below. **This is orchestrator prose-writing — dispatch NO agent and render NO verdict.** The summary is a 1-page section-templated narrative; `references/summary-format.md` documents the skeleton.

**First, the task-file read (the Review guide's input).** Take the task-file list PHASE 2.2 assembled and read each file for two things: its title, and any **behavior-changing / behavior-preserving labels its `## Change Details` entries carry**. Those labels are the Two-hats partition `/devforge:breakdown` writes for a MIXED task — the labels sit ON the `- In <path>:` entries already in that section (and, where one file holds both kinds of change, on the named functions under an entry). **There is no heading to look for: that lane adds no subheading, no header field and no flag**, so the partition is visible only as the labels themselves. Only a mixed task carries them, so most task files have none — their absence is normal and is not a defect to report. This read is direct because those labels are free-form prose no helper verb parses; there is no scratch file for them.

Then compose these eight sections (each user-facing, 1-5 lines; describe behavior and outcomes, not implementation mechanics). **Three are conditional — 2, 6 and 8 are omitted ENTIRELY when their condition says so, and a feature that trips none of the three renders the other five:**
...
2. **Review guide** — **OMIT THIS SECTION ENTIRELY when there is nothing to orient by** (`$WORKDIR/decisions.json`'s `decisions` is empty AND no task file's `## Change Details` carries a behavior-changing / behavior-preserving label). When present, 3-5 lines telling a reviewer where to start, which files carry which decision, and which changes follow mechanically from those decisions. Compose it from the `decision` / `chosen` / `rationale` fields, the labels read above, and task titles + `files_changed` from `$WORKDIR/notes.json` — **and from nothing else.** The mapping from files to decisions is your judgment over those named inputs and **nothing checks it**: the section orients a reader, it does not certify anything about the code, so never state or imply that it is complete.
3. **Changes** — one line per task, drawn from `$WORKDIR/notes.json` (the task's accomplishment, summarized in user terms — group by what changed, not raw file paths).
4. **Files changed** — the directory grouping from `$WORKDIR/changes.json`'s `by_directory`, plus the `git diff --stat` totals (`insertions` / `deletions`, or the `stat_summary` line). When `changes.json`'s `source_changes` is non-null (wrapper mode, PHASE 1), include the source-repo change set too (D5).
5. **Key decisions** — the most important decisions from `$WORKDIR/decisions.json`'s `decisions` (one line each: what was decided and why, from the `decision` / `chosen` / `rationale` fields).
6. **Deviations from plan** — **OMIT THIS SECTION ENTIRELY if no task noted a deviation** (no task in `$WORKDIR/notes.json` has a non-empty `notes`). When present, one line per deviating task.
7. **Acceptance criteria** — a compact checklist, with each AC's status taken VERBATIM from `$WORKDIR/verification.json`'s `ac_list` (NOT re-derived from the spec). When `verification.json` is the empty-fallback from 2.1, render the unverified note instead of a checklist.
8. **Not verified** — **OMIT THIS SECTION ENTIRELY when it would be empty**: no task in `$WORKDIR/notes.json` has `has_unverified` true, AND `verification.json`'s `e2e_status` is null or `e2e-clean`, AND both `has_scope_creep_advisory` and `has_leftover_artifacts_advisory` are false. When present, one line per surviving item — the tasks with unverified Done-When boxes, the `e2e_status` value when it is NON-NULL and not `e2e-clean`, and each advisory that is flagged. A null `e2e_status` contributes no line and is never rendered as a value (see 2.1 — the no-suite and `off` cases both arrive as null). **Every line QUOTES a status `/devforge:verify` or `/devforge:implement` already recorded; none is a judgment made here.** Say of the e2e and advisory lines that they did not block the verdict, because that is what `/devforge:verify` recorded about them. Do NOT repeat non-passed ACs — section 7's checklist already annotates each one, and this section names only what that checklist does not.

**The `**Run by**:` provenance line.** Above those eight sections, on the line directly under the summary's title, `summary.md` may carry one provenance line recording who ran the command that CREATED this summary. It is document provenance, not feature content — it is composed from the two sources named below and from nothing in the `$WORKDIR` scratch inputs, so the no-speculation rule below is not in tension with it. No helper composes it: this synthesis is inline (D6), and there is no `summarize_helper` verb for the line (do not add one). Resolve it in two steps:
...
Keep it concise: this is a summary, not a report. Deduplicate — group files by area rather than listing each. Do not speculate — include only what is present in the spec, plan, task `## Completion Notes`, a task's title or its `## Change Details` section, `verification.md`, or git data.
...
4. **No speculation** — include only information present in the spec, plan, task `## Completion Notes`, a task's title or its `## Change Details` section (the behavior-changing / behavior-preserving labels PHASE 3 reads), `verification.md`, or git data. Do not infer or guess. This rule governs the eight SECTIONS; the header's `**Run by**:` provenance line is not feature content and is composed under PHASE 3's own rules for it.
...
9. **The two NEW conditional sections orient and quote — they never judge** (Deviations, the third conditional section, is unchanged by this rule) — the Review guide is orientation over named inputs (`decisions.json`, the behavior-changing / behavior-preserving labels on task `## Change Details` entries, task titles + `files_changed`) and **nothing checks its mapping**, so it never claims completeness. The Not-verified section quotes statuses `/devforge:verify` and `/devforge:implement` already recorded (`has_unverified`, `e2e_status`, the two advisory bools) and adds no item of its own — it is a report of what was not verified, never a finding about it, and rule 6 still holds across both.
