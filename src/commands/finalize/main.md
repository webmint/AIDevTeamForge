...
- **Install/wrapper repo** — `feat(<feature-name>): <title>`, where `<feature-name>` is the identity segment of `<feature_dir>` — the slug after the `<NNN>-` prefix for a feature directory allocated before the `<YYYY>/<MM>` bucketed layout, and the whole last segment for one allocated under it, which carries no `<NNN>-` prefix to strip — and `<title>` is the feature's title, drawn from the spec's `## 1. Overview` section (the first 1-2 sentences, condensed to a commit-subject-length title). Both shapes are in use and nothing migrates either, so take the segment from `<feature_dir>` as PHASE 0.1 resolved it rather than expecting one shape. The `squash` verb APPENDS `COMMIT_ATTRIBUTION` per config — do NOT add attribution to the subject yourself. Before calling `squash --confirm` (3.4), assert the composed `<title>` (from the spec's `## 1. Overview`) is non-empty; if the Overview is blank, ask the user to supply a commit title before proceeding — an empty `--install-message` would produce a malformed `feat(scope):` commit (the helper does not validate non-emptiness).
