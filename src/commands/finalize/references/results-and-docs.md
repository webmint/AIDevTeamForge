# /devforge:finalize — tech-writer doc targets + results-block format
...
NEVER point the agent at `docs/features/`, `docs/api/`, or `docs/guides/` — those tiers are dropped under Plan F (see `tech-writer.md`'s Plan-F notes). A new concern (a new `src/` subfolder), a new API surface, or a new domain term is NOT hand-authored here — those are left to `/devforge:generate-docs` on its next run, which is why PHASE 4 prints a soft `/devforge:generate-docs` reminder for structural doc drift this surgical pass does not cover.

### Why both `/devforge:finalize` and `/devforge:generate-docs` touch `docs/`

`/devforge:finalize`'s tech-writer pass does TARGETED, feature-driven surgical updates at feature-completion time (Normal mode, scoped to the feature's changed files). `/devforge:generate-docs` does stamp-gated whole-codebase regeneration (tech-writer Skeleton-Fill mode). Both exist on purpose: `/devforge:finalize` keeps a shipped feature's docs current without forcing a whole-repo regen; `/devforge:generate-docs` is the canonical structural author. The PHASE-4 reminder is the soft pointer between them.
