...
Per-rule tag selection: rules cited from `architecture.md` → `extracted`; rules implied by tooling configuration (tsconfig strict, ESLint rules, etc.) → `enforced`. See `references/section-shapes.md` § "Tag distribution" for the full decision rubric.
...
Enabling `design_token_provenance` turns on a static provenance check — no hardcoded color / border / spacing literals, no `var(--x, <literal>)` fallbacks, undefined tokens fail loud, and `:hover` + `:focus-visible` required on interactive elements — that runs at the `/devforge:implement` per-task gate and the opt-in pre-commit hook. It is the build-time half of the constitution's Design Fidelity principle.
...
# flag is passed — the design_token_provenance detector reads no manifest, so
# --manifest-path is retained for config back-compat only and its value is never read.
...
`validate` runs the 4-dimension content quality framework and emits a JSON report on stdout — `composite`, `dimensions` (a `score` + `pass` pair per dimension), `failed_items`, and `filtered_items` (one entry per citation token a Dimension-2 filter dropped, naming the filter class that matched it; empty when nothing was filtered). Composite ≥ 0.95 → exit 0 (pass); below → exit 2 with stderr enumerating per-dimension scores + failed items. The four dimensions (weight in parentheses):

1. **Slot-fill** (0.30) — required sections / fields populated (9 slots for `existing-codebase`, 10 for `greenfield`).
2. **Citation validity** (0.25) — path-like tokens in rule text / table cells / code-example annotations resolve relative to install root (a wrapper install also tries the wrapper-declared `project_root`); package-name lookup via `INIT_JSON.packages_detected[]`. A token that neither the direct-path check nor the package-name lookup resolves gets a recursive search under the search root — install root for a standalone install, the wrapper-declared `project_root` joined onto the install root for a wrapper install — pruned at any depth by a fixed directory-name exclusion set (`.git`, `node_modules`, and common build-output / dependency / cache directories); a slash-free token also resolves against any file whose name ends with it. Four token classes are filtered rather than counted — convention placeholders (`PageXxx.vue`), extension-only fragments (`.spec.ts`), leading-slash fragments (`/index.md`) and cross-slash artifacts (`.ts/.vue`) — so they land in neither the numerator nor the denominator and are reported under `filtered_items`. A token whose first path segment is the framework's runtime directory (`.devforge`) counts resolved with no filesystem check: the pipeline creates those files later.
