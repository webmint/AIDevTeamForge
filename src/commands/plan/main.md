...
Before beginning the plan work, check for a `/devforge:grill` re-entry seed. Glob `specs/*/grill-seed.json`. If any matched file has a `target_stage` equal to `"plan"` (this command's stage), you are re-entering from a `/devforge:grill` REVISE-PLAN disposition: the design-time grill proved correctable defects in this feature's `plan.md`, and this re-run must address the confirmed findings rather than re-derive the flawed plan. Read the seed DIRECTLY: parse the matched file's flat JSON inline — do NOT call any grill helper or `plan_helper` verb to read it (the read stays helper-free so this block remains valid even if `/devforge:grill` is later removed). The seed carries these fields:

- `feature` — the feature this directive applies to; state it up front (do NOT infer it from the file path).
- `prior_conclusion` — the flawed plan decision `/devforge:grill` invalidated; do NOT repeat it in the revised plan.
- `invalidating_evidence` — how `/devforge:grill` proved that plan decision wrong (grounded in `plan.md` / `spec.md` / code).
...
7. Does any Key Design Decision RESTRICT existing behavior of shared code (a shared service, utility, or layer with multiple callers)? Per the constitution's Narrowing rule (Design Principles section), return one line per such decision: is the restriction caller-scoped (an opt-in the affected caller passes) or layer-wide? For a caller-scoped one, name every caller classified in-scope in the carried caller enumeration (or re-derived per the trigger below; or, when no `**Caller enumeration**` section was carried at all, established by the architect tracing the callers directly) as the callers that receive the opt-in wiring, and confirm no in-scope caller is left unwired — a caller-scoped restriction whose needing-caller set is unnamed is not recordable. For a layer-wide one, return the list of every current caller it affects (for the decision's row) and the reason a caller-scoped opt-in genuinely can't cover it. When the needing-caller set can't be established by any of those paths, a layer-wide key on data every caller already passes is the failure-tolerant fallback (per the constitution's Narrowing rule) — record that instead. A decision that restricts no shared code returns nothing here.
...
- **`cancel`** → tell the user `"/devforge:plan cancelled. Plan draft preserved at specs/[feature-name]/plan.md."` and end the turn.
...
- Exit 0 → the helper wrote `specs/NNN-<feature>/plan-handoff.json` and printed its path on stdout. Surface the written path to the user in one line, e.g. `"Structured plan handoff written: <path> (consumed by /devforge:breakdown Phase 0 via breakdown_helper read-plan-handoff)."`
