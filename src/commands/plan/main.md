...
On the BLOCKED path this run is over. Offer to run `/devforge:spec-check <spec-path>` now and end the turn; run it only once the user agrees, and never re-invoke `/devforge:plan` on that same agreement — when the check finishes, propose `/devforge:plan` as its own next step, which restarts at Phase 0a. One agreement covers one command.
...
What the line recommends is a tier, resolved through this install's own `/devforge:configure` answers, so it tracks the user's choice and names no model version. Then proceed to PHASE 0 (Research Evaluation) with the resolved path.
