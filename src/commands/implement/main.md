...
`<feature_dir>` — here and everywhere else in this document — is the feature directory holding the task currently in flight: one path the orchestrator holds in working memory. PHASE 1 binds it: `resolve-next-task` emits a `feature_dir` value on its `{"state": "task", ...}` object, and that value *is* `<feature_dir>`. Hold it exactly as the helper reported it — do not re-shape it, do not rebuild it from `task_file`, `index_file`, or any other key on that object, and do not spell what is inside it. This command's per-feature artifacts are `<feature_dir>/tasks/` plus a filename; the `.devforge/` outputs below live at the install root and are not under `<feature_dir>` at all. Two things are particular to this loop: the binding is re-made on EVERY PHASE 1 pass, so it can name a different directory once a feature's tasks are all drained; and PHASE 0 runs before the first PHASE 1, so nothing is bound while crash recovery runs — that phase takes the interrupted run's directory from the `**Feature**` field of `.devforge/wip.md` instead.
...
- Updated `tasks/<NNN>-<title>.md` (Status `Complete`, ticked Done-When boxes, filled Completion Notes) + `tasks/README.md` index row per approved task. When verification was scoped (PHASE 5 `tooling_unavailable` → `scope-and-approve`), the type-check / lint / test Done-When boxes are left unticked and annotated `_(unverified — see Completion Notes)_` instead of ticked; the standing test box alone is left unticked and annotated the same way on a clean `pass` whose `test_commands_run` array is empty (PHASE 5), since no test command ran.
...
`/devforge:implement` runs AFTER `/devforge:breakdown` produces the task set, BEFORE `/devforge:review`. It consumes the structured `<feature_dir>/breakdown-handoff.json` (the producer-side handoff written by `/devforge:breakdown`) as its read contract: the orchestrator never re-derives task structure from the markdown task files — it reads the machine contract via `resolve-next-task` and the markdown task body via Read.
...
**This branch runs once, before the first `resolve-next-task`, and is the SOLE interrupted-session detector.** Per-task preflight (PHASE 2) only asserts that `wip.md` is absent; it does not offer recovery. See `references/crash-recovery.md` for the full mechanics.
...
The helper checks (in order): constitution populated (`constitution.md` lacks the `_Run /devforge:constitute to populate_` sentinel — the check also matches the legacy un-namespaced `_Run /constitute to populate_` that an older install may still carry), a feature branch is checked out in the **source** repo (refuses `main`/`master`/`trunk` and the source repo's origin default branch — that is where code commits land), no stale `.devforge/wip.md` remains in the install root, and a readable git HEAD exists in the source repo. On success it emits JSON `{constitution_digest, memory_present, memory_excerpt, memory_state, head_sha, branch, source_branch, source_dirty_warning}` on stdout (exit 0). `memory_excerpt` is the populated `## ` sections of `.devforge/memory.md`, `## Task Outcomes` excluded — a content-free section is dropped heading and all, and any truncation is declared by a marker line under the affected heading. It is NOT the carry source for the implementing agent's brief: PHASE 3 selects the entries that bear on THIS task, per `references/agent-brief.md`. `memory_present` and `memory_state` (`absent` / `stub` / `populated`) are the probe results over that same file; on `absent` or `stub` no lesson is recorded yet, so PHASE 3's Edge-cases bullet is simply absent and you say nothing about memory. `head_sha` is the **source** repo HEAD; capture it as the task's **checkpoint SHA** — the rollback target for `skip` and recovery. (`constitution.md` and `.devforge/` are install-root artifacts; the helper reads them from the install root and runs the branch/HEAD/dirty git checks against the source repo, resolving `<source_root>` internally from the install-root `--root`.)
...
2. Write `.devforge/wip.md` with the mandatory `**Command**: /implement` field plus `**Feature**` (`<feature_dir>`, the whole path — PHASE 0's `skip` arm joins `tasks/` onto this value), `**Task**` (number), `**Title**`, `**Agent**`, `**Phase**` (set to the phase about to run, starting at `dispatch`), and `**Checkpoint**` (the checkpoint SHA captured above). The marker is the crash-recovery anchor read by PHASE 0 on the next run. See `references/crash-recovery.md` for the field layout.
...
The agent edits **source** files. The handoff's `touched_files` are source-root-relative, so give the agent source-rooted paths (`<source_root>/<touched_file>`) and tell it to operate under the source root. State explicitly that the agent must NOT write forge artifacts (`.claude/`, `specs/`, `CLAUDE.md`, `constitution.md`, `.mcp.json`, `docs/overview.md`, `docs/architecture.md`, `bugs/`, `research/`) into the source tree — those live at the install root. `research/` is a LEGACY entry kept for grandfathered installs (plan 68 moved intake artifacts into the feature directories under `specs/`, already covered by `specs/`); `discover/` was never in this list, and the clean cut moots that asymmetry rather than fixing it — mirroring the helper's `ISOLATION_ARTIFACTS` comment. In wrapper mode PHASE 5's wrapper-isolation check fails the task if any such artifact appears inside the source root. In standalone mode `<source_root>` is the install root, so source-rooted paths are just the repo-relative paths as today and the isolation rule is moot (the single repo legitimately contains those artifacts).
...
- **`{"status": "pass", ...}`** (exit 0) → verification passed. The object carries `commands_run` (the static type-check + lint commands), `build_commands_run`, and `test_commands_run` — the de-duplicated commands the helper actually ran in each group. `test_commands_run` is EMPTY when none of the touched files resolved to a configured test command, so a `pass` with an empty `test_commands_run` means verification succeeded without a single test running. Capture it; PHASE 7's approve path reads it. Proceed to PHASE 6.
