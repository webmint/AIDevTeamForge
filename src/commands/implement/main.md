...
When PHASE 5 returns `tooling_unavailable`, a configured type-check/lint command could not be executed, so the mechanical verify gate cannot complete. This is DISTINCT from the Gate-blocked path above: the change is not failing on a code error, it is unverifiable by the type checker — so this path CAN reach `approve` (via `scope-and-approve`), whereas the Gate-blocked path never can. After relaying the helper's stdout VERBATIM (per PHASE 5), ask via `AskUserQuestion`:

- Question: `"Task NNN's type-check/lint command can't run — how to proceed?"` — single-line text (substitute the resolved `number`).
