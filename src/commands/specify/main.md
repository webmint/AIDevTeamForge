...
- Branch `spec/<feature-slug>` — FALLBACK only. `/devforge:research` and `/devforge:discover` create the spec branch at intake — `spec/<ticket>` when the run named a ticket, `spec/<feature-slug>` when it did not — so `/devforge:specify` normally creates nothing; Step 4.1 calls `create-branch` only when Phase 0.2 deferred creation, and only its default-branch arm emits a checkout.
...
Branch decision. `/devforge:research` and `/devforge:discover` create the spec branch themselves at intake when the run is on the default branch — `spec/<ticket>` when the run named a ticket, `spec/<feature-slug>` when it did not — so the first arm below is the normal case and `/devforge:specify` creates no branch. The other two arms are the fallback states: intake skipped the checkout (not a git repository, unresolvable default branch, or intake itself ran on a non-default branch), or the user switched branches since.
...
On `import-handoff` exit 0 with a `warning:` line on stderr (prefixed `import-handoff: warning:`) (re-import would overwrite user-composed `state.overview` / `state.desired_behavior` / AC content): surface the stderr warning text to the user as plain prose alongside the verbatim stdout block, then continue to Phase 0.5. The helper has already overwritten pre-seeds; the warning is informational so the user knows downstream sections may need re-review at Phase 5 approval.

### Phase 0.5 — Re-entry from a downstream re-entry seed (conditional — skip if no seed)

Before beginning the spec work, check for a re-entry seed. When Phase 0.4's chosen hit carried the ` | re-entry` marker, the seed that admitted it is in the resolved feature dir and this block WILL fire; when the hit was unmarked, it normally does not. Either way the check is the same and is run unconditionally: glob `<feature_dir>/*-seed.json` — the feature dir resolved in Phase 0.4, and ONLY that dir. `<feature_dir>` already carries every segment above the filename, so nothing is prepended to it. Do NOT glob `specs/*/` project-wide: another feature's stale spec-targeted seed would then bind this run to a directive that was never about this feature, and the same-dir scoping is exactly what Phase 0.4's arm (b) already keyed on. That scoping is also why this one check stays a glob the orchestrator runs itself instead of a `.devforge/lib/artifact_helper find-feature-artifacts` call the way §1.7's prior-spec enumeration does: that verb searches EVERY feature directory and carries no flag that narrows it to one, so routing this check through it would perform the project-wide enumeration this block exists to forbid. If any matched file has a `target_stage` equal to `"spec"` (this command's stage), you are re-entering this (upstream) spec stage because a DOWNSTREAM command concluded that this stage's output is what must change; read the seed's `source` field to know which command emitted it — the `source` bullet below names what each value means. Whatever the source, a downstream command invalidated a conclusion of THIS spec stage, so the re-run must be DIRECTED and must not re-derive the invalidated conclusion. Read that seed and treat it as a binding directive for this run. Read it DIRECTLY: parse the matched file's flat JSON inline — do NOT call any producing command's helper verb (the orchestrator reads the file itself, so this block stays valid even if any producing command is ever removed). The seed carries these fields:

- `source` — which command emitted this seed; read it first and state up front what you are re-entering from. The values `SEED_SOURCES` accepts are `grill` (a `/devforge:grill` RE-ENTER-UPSTREAM verdict on the plan's design), `spec-check` (a `/devforge:spec-check` REVISE-SPEC verdict on the spec's AC logic), and `fix` (a `/devforge:fix` scope-change bounce — its PHASE-1 triage concluded a working-list item is a feature or architecture change, not a defect repair); read the field rather than assuming that list is closed.
...
- `invalidating_evidence` — how that downstream command proved it wrong, grounded in the plan / spec / code.
...
State up front in your first user-facing message that you are running in re-entry mode — naming the seed's `source` command, read from the field — for the named `feature`, and name how this run addresses `must_satisfy`. Then proceed to Phase 0.6, and run Phases 1–5 with the seed's directive constraining the spec.
...
When no `<feature_dir>/*-seed.json` file matches `target_stage == "spec"` (the normal case — a producing command writes a seed only when the user picks, at that command's own human gate, the arm matching its recommended re-entry disposition, and most runs never reach one), this block is a no-op: proceed directly to Phase 0.6.

### Phase 0.6 — Model advisory (printed, never gating)
...
### 1.7 `specs/` (if any prior spec exists) — prior spec directories on related topics

Enumerate the feature directories that already hold a spec:

```bash
.devforge/lib/artifact_helper find-feature-artifacts --filenames '["spec.md"]'
```

Stdout is a JSON object; take `matches` from it — one entry per feature directory holding a `spec.md`, each carrying `feature_dir` (that directory) and `file` (the `spec.md`'s own path, ready to read). Read the `spec.md` of any entry whose directory name — the last segment of its `feature_dir` — has ≥1 topic-token overlap with the feature description. A feature directory holding no `spec.md` is not listed, and has nothing for this section to read. An EMPTY `matches` means no feature directory holds a `spec.md` yet: the call exits 0 in that case, because finding nothing is a normal outcome and not a failure, so there is no exit code to test for it — branch on the array being empty and read nothing in this section. A non-zero exit means the call itself failed (a malformed `--filenames` value, or a workspace that could not be resolved), never that no prior spec exists: copy the helper's stderr VERBATIM into your next user-facing message as a fenced code block (do not summarize or paraphrase), then end the turn.

Record one `record-input-read` call per file consumed, passing that entry's `file` value to `--path` exactly as reported — install-root-relative, the shape the path auto-tag needs; never re-shape it and never substitute a path composed from another value. Auto-tagged `source_origin = "prior_spec"`.
...
On three of the four paths below, `spec_number` + `feature_slug` decide which directory `spec.md` is written into, so wherever Phase 0.4 resolved a feature dir they must name THAT dir rather than a fresh one; the fourth resolves no number at all and writes into `<feature_dir>` as Phase 0.4 bound it. Take exactly ONE of the four number paths below — they are ordered; use the first whose condition holds — then run the feature-name, date, and branch steps that follow, which apply to all four. A re-entry revision (Phase 0.5 fired) takes the same paths on the same conditions: an existing `spec.md` in the resolved dir changes nothing here, because these paths key on state and on the directory's name.
...
**Bucketed path — state carries no `spec_number`, and the resolved feature dir sits in a `<YYYY>/<MM>` bucket** (its parent directory's name is exactly two digits and its grandparent's is exactly four — the same ancestry `import-handoff` already applies when it seeds identity from that directory). Intake allocated this directory under those two date levels and gave its last segment the feature's own identity, so there is no `<NNN>` in the name to split out and none to invent. Reuse `<feature_dir>` exactly as Phase 0.4 bound it and allocate nothing — `spec.md` belongs beside the intake handoff already sitting there. Call NEITHER number setter: `set-spec-number` has no value to take, and `assign-spec-number` would allocate a number that names no directory this run writes into. `spec_number` stays unset for the rest of the run.

The last segment holds one of two things, and only the feature-name step below is affected by which:

- **A 2-4 word kebab-case slug** (e.g. `dark-mode-toggle`) — this feature's slug, already seeded into `feature_slug` by `import-handoff` on the two importing arms. Pass it verbatim below.
- **A ticket identifier** (e.g. `PROJ-123`) — not a slug, and not to be turned into one: `assign-feature-name` rejects it, and re-casing or otherwise reshaping it would put an identity nobody typed into the spec header and the branch name. `import-handoff` seeds neither field from such a directory. Compose the feature name from `$ARGUMENTS` below instead.

**Ordering — this path is tried after the warm and cold paths and before the genuine fallback, and both halves carry weight.** After, because `import-handoff` reads a `<NNN>-<slug>` last segment first and unconditionally, so a directory those two paths can serve is served by them whatever it sits inside. Before, because a bucketed directory also satisfies the genuine fallback's condition, and that path would allocate a fresh directory and leave the intake report and handoff orphaned in this one.

One documented limitation of this path, the counterpart of the one the genuine fallback records below: when the last segment is a ticket identifier, the rendered spec header will not name the directory `spec.md` sits in. `assign-feature-name` rejects that identifier, so the feature-name step below composes a name from `$ARGUMENTS`, and `render` writes that name — and no number — into the spec's `# Spec:` header, while the directory keeps the ticket intake gave it. Nothing reconciles the two: no step renames a feature directory, and none may, because that name is the one intake allocated and the intake artifacts already sit under it. When the last segment is a kebab-case slug instead, the two agree — that segment is passed to `assign-feature-name` verbatim. Tell the user this run carries no spec number when you take this path, and say that the header and the directory name will differ when the segment is a ticket.
...
**Feature name (all four paths).**

```bash
.devforge/lib/specify_helper assign-feature-name --feature-name "<2-4 word kebab-case>"
```

On the warm and cold paths, `<slug>` is the resolved feature dir's basename after the `<NNN>-` prefix — pass it verbatim so the rendered header matches the directory. On the bucketed path it depends on the last segment: pass that segment verbatim when it is itself a 2-4 word kebab-case slug, and when it is a ticket identifier instead — which this helper rejects — generate a name from `$ARGUMENTS` exactly as the genuine-fallback sentence next directs, accepting that the rendered header will then not match the directory name. On the genuine-fallback path, generate a 2-4 word kebab-case slug from `$ARGUMENTS` (e.g., `user-auth`, `dark-mode-toggle`) and, if the proposal fails validation, fix it and re-call. The helper validates the pattern `^[a-z][a-z0-9]*(?:-[a-z0-9]+){1,3}$` and persists both `feature_name` and `feature_slug`.

This call runs on the warm path too, even though `import-handoff` already seeded `feature_slug`: the import seeds `feature_slug` alone, and `feature_name` is what the rendered spec title and the Step 5.1 approval summary use. Where intake used the slug as the directory's name, that name passes validation, because intake validates the slug against this same pattern before using it. If the helper nonetheless rejects a dir-derived slug (a hand-created or pre-migration directory), copy its stderr VERBATIM into your next user-facing message as a fenced code block (do not summarize or paraphrase) and end the turn — the user renames the directory or re-runs intake.
...
**Branch — fallback only.** `/devforge:research` and `/devforge:discover` create the spec branch at intake — `spec/<ticket>` when the run named a ticket, `spec/<feature-slug>` when it did not — so the session is normally already on it and nothing runs here. Call `create-branch` only when Phase 0.2 deferred branch creation — the default-branch arm, or the `from-here` choice on the other-branch arm:
...
When on the default branch, the helper's stdout emits a single line of the form `git checkout -b spec/<feature-slug>`, composed from the `feature_slug` the feature-name step above just set. Execute that line:

```bash
git checkout -b spec/<feature-slug>
```

Tell the user: `"Created and switched to branch spec/<feature-slug>"`. That slug is the one now in state, so on the warm and cold paths it is the slug half of the resolved feature dir's name, and on the bucketed path it is that directory's last segment whenever that segment is a slug; where the segment is a ticket identifier instead, the branch carries the name composed from `$ARGUMENTS` and the two differ, as that path's own limitation note says of the spec header. When on a non-default branch (Phase 0.2 user choice = `from-here`), the helper emits a `# already on non-default branch ...` informational comment and skips the checkout.
...
Write the rendered bytes into the feature directory. On the warm, cold and bucketed paths that directory is the one resolved in Phase 0.4 and already exists — the `mkdir -p` is a no-op there and creates the directory only on the genuine-fallback path:
...
The helper writes `<feature_dir>/design-anchor.json` atomically from the design anchor already in `/devforge:specify` state — the one carried from a `/devforge:research` / `/devforge:discover` handoff by `import-handoff` (Phase 0.4) when intake captured one, or, when intake captured none, one the helper composes from the `**Design source**:` declaration recorded by `set-design-source` (Step 4.10) as a backstop. The backstop is internal to the helper; no separate orchestration is required. Its preconditions are `feature_slug`, plus EITHER a `spec_number` OR a state shape from which it can resolve the bucketed feature directory. `assign-feature-name` sets `feature_slug` on every Step 4.1 path, and the warm, cold and genuine-fallback paths each set `spec_number` too, so on those three the number satisfies the second half. On Step 4.1's bucketed path no `spec_number` is set and the helper resolves the directory instead, from the intake handoff path Phase 0.4 recorded in state (`import-handoff` on the two importing arms, `record-handoff-path` on `cold`) — the same directory Phase 0.4 resolved, read back rather than composed — and writes the anchor beside the intake artifacts already sitting there. The helper creates the target directory itself if needed. The `**Design source**:` frontmatter line stays as the human-readable summary of the same intent; `design-anchor.json` is persisted as the structured, immutable design-intent record, so later pipeline stages can read it directly in place instead of having it re-carried through handoffs. `/devforge:specify` is the gate that guarantees this record is persisted before `/devforge:plan`; the write is idempotent — a re-run of `/devforge:specify` overwrites it. On a non-zero exit, copy the helper's stderr VERBATIM into your next user-facing message as a fenced code block (do not summarize or paraphrase), address the cited cause, and re-run.
...
.devforge/lib/artifact_helper commit-artifacts \
    ...
    --label "spec: <feature-dir-name>"
```

`<feature-dir-name>` in that label — and in the WIP-commit block below, which composes the same label — is the last segment of `<feature_dir>`: the directory's own name, not its path. The label names the directory this run wrote into, so it resolves on every Step 4.1 path — on the warm, cold and genuine-fallback paths that segment is the `<NNN>-<feature-name>` pair, and on the bucketed path it is the identity segment intake gave the directory, which has no `<NNN>` to split out and none to invent.
