...
`/devforge:configure` is the third command in the 4-command sequence (`/devforge:init-forge` → `/devforge:generate-docs` → `/devforge:configure` → `/devforge:constitute`). It consumes the structural fields persisted by `/devforge:init-forge` and the docs corpus produced by `/devforge:generate-docs`, fills 36 configuration fields via `.devforge/lib/configure_helper` setters, prunes `.claude/agents/*.md` against the project's natures, renders the consolidated config, substitutes `{{KEY}}` placeholders in the framework's templates, and applies the configured per-tier models to the agent and command files.

## Outputs of this phase

- `.devforge/configure.yaml` — canonical state (37 fields). Owned + shaped by the helper; mutated only via setter subcommands.
- `.devforge/project-config.json` — render artifact rebuilt from `configure.yaml` + `.devforge/init.yaml` + `.claude/agents/` listing on every run (45 keys: 37 from configure.yaml + 5 from init.yaml + 3 derived).
...
- `.claude/commands/devforge/*.md` — the eight commands the apply verb maps to a tier carry `model:` / `effort:` frontmatter written from the same Q11 answers (Phase 5.4). Nothing else in a command file is touched, and no other command file is touched at all.
...
These twelve fields cannot be derived from filesystem scan; each requires a user choice. One AskUserQuestion per question, in order — with one exception: Q11 makes one call per tier, four calls in all, each carrying TWO questions (that tier's model and that tier's effort level), per `references/q11-tiers.md`. Persist each answer via its setter before issuing the next question.
...
Probe first, then ask. Before Q11.1, dispatch one minimal subagent per Claude Code model alias to find out which ones this session can dispatch on, and report the four results in one line; an alias that fails the probe is not offered as an option. Then make four AskUserQuestion calls — Q11.1 (think), Q11.2 (do), Q11.3 (verify), Q11.4 (security) — each carrying two questions, that tier's model and that tier's effort level, for eight answers persisted through eight setters (`set-claude-tier-<tier>` + `set-claude-effort-<tier>`). See `references/q11-tiers.md` for the probe procedure, the full prompt text and options per tier, why the security tier is asked separately from the verify tier, the arm for a session where fewer than two aliases are available, the route for pinning a model that was not offered, and the reasoning behind each recommendation.
...
Once `configure.yaml` is fully populated (36 fields set), render the consolidated JSON config, prune the agents directory against `project_natures`, substitute the templates, then apply the Q11 model and effort answers to the agent files that remain and to the eight mapped command files. Four sub-steps in fixed order: render-config → prune-agents → substitute-templates → apply-models. The order matters: `render-config` derives `AGENT_LIST` from the on-disk agent listing, so any pruning must happen AFTER `render-config` writes the snapshot to `project-config.json` (otherwise the substituted `{{AGENT_LIST}}` would still advertise dropped agents). `substitute-templates` then walks the post-prune file set, so dropped agents are not substituted. `apply-models` runs LAST so it rewrites the post-prune, post-substitution agent files, keyed on the `model_tier:` line each one carries; the command files it also rewrites are untouched by the three sub-steps before it, so for those the ordering constraint is not what puts it last.
...
### Phase 5.4 — Apply models

```bash
.devforge/lib/configure_helper apply-models
```

`apply-models` reads `.devforge/project-config.json` and rewrites `model:` / `effort:` frontmatter in two classes of file. (`apply-agent-models` stays registered as an alias of this verb for one release; the name above is the one to use.) The framework itself picks no model for any tier — everything a file's `model:` line says comes from the Q11 answers, and a tier nobody answered leaves that file on the model the session is already running.

- **Agents** — every `.claude/agents/*.md` file remaining after Phase 5.2 that carries a `model_tier:` line. `model:` is set from that tier's `CLAUDE_TIER_*` value; when that value is absent or null the line is set to the literal `inherit`, Claude Code's documented value for keeping the active model. A file with no `model_tier:` line — an agent this project wrote itself — is left byte-identical and reported under `skipped[]`.
- **Commands** — the eight `.claude/commands/devforge/*.md` files the verb's own map assigns a tier: `specify`, `plan`, `grill` and `breakdown` on `think`; `implement` and `fix` on `do`; `review` and `verify` on `verify`. `model:` is set from that tier's `CLAUDE_TIER_*` value; when that value is absent or null NO `model:` line is written at all, so nothing overrides the model the session is on. A command outside that map is left byte-identical and reported under `skipped[]`. A command's `model:` line is a turn-scoped override — it applies for the rest of that command's turn, and the session's model resumes at the next prompt.

In both classes `effort:` is set from that tier's `CLAUDE_EFFORT_*` value, and the line is REMOVED when the value is `default`, which is what makes that file inherit the session's own effort level. Any file whose frontmatter block is missing or unterminated is left byte-identical and reported under `skipped[]`. No body byte is touched in any file. The verb is idempotent: running it twice changes nothing the second time.

Stdout is a JSON report with two keys. `applied[]` carries one entry per rewritten file, each with `kind` (`agent` or `command`), `name` (the file's stem), `tier`, `model` (`null` only for a command on an unconfigured tier, since that is the one case where no `model:` line is written), `effort` (`null` when the file is left with no effort line), and `changed` (`false` when the file was already correct). `skipped[]` carries one `{kind, name, reason}` entry per untouched file. Both lists are sorted by kind then name, so agents come before commands. Exit codes:

- Exit 0 → the report was emitted. Print a compact `name → model / effort` list built from `applied[]`, grouped by `kind` (write `-` for a null model or a null effort), followed by the names in `skipped[]`, so the user sees which files moved and which were left alone.
- Exit 1 → `.devforge/project-config.json` is missing or malformed, or a file in either class could not be read or written. Surface stderr verbatim and ABORT.
- Exit 2 → at least one file failed validation (an unknown `model_tier:` value, a `CLAUDE_EFFORT_*` value outside `default | low | medium | high | xhigh | max`, a duplicated `model_tier:` / `model:` / `effort:` key inside one file's frontmatter, an agent's `model_tier:` line with no sibling `model:` line, or — commands only — a frontmatter block with no `description:` line to anchor an inserted `model:` / `effort:` line after). Stderr names the offending file or configuration key. NOTHING was written — surface stderr verbatim and STOP; fix the named value and re-run this sub-step.

`update.sh` runs this same verb under the same configuration guard as placeholder substitution, so a configured project keeps its model and effort choices across framework updates instead of depending on how a merge resolved each `model:` line. It runs it AFTER the step that re-emits the promoted commands: that re-emit overwrites every command file wholesale from the framework's own sources, so an apply placed before it would be erased later in the same run. Before merging the regenerated agents it also runs the verb against the snapshot under `.devforge/template/`, so the merge baseline already carries this install's configured values and a consumer's own choice never conflicts with the `inherit` line the framework emits. Its summary names both classes — `Applied models: agents N (M changed), commands K (L re-applied after re-emit), skipped S` — followed by one line per changed file, agents and commands alike. The command count does not trend to zero the way the agent count does, and a non-zero `L` on a settled install is not a sign that anything drifted: the re-emit strips every mapped command's `model:` / `effort:` line moments before this call writes it back, so each of those files legitimately changes on every single update. A failure there is reported rather than fatal: the files keep their current lines and the update continues. An install whose framework files predate this build gets that call on its next `update.sh` like any other, since the script ships with the framework rather than with the project.
