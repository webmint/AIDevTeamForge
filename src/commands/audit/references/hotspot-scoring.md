...
The helper always emits the positions N+1..N+10 by score as the "Next 10 Candidates" tail (OQ-5 resolved). `render-hotspot-summary` renders each as `file · score · (churn, callers, size)`. This appears in the report appendix (see `references/report-format.md`) so the user can sanity-check what was almost picked and re-tune weights if the ranking looks wrong.
