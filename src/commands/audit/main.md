...
The same stdout JSON carries `memory_present` (bool) and `memory_excerpt` (the first 40 lines of `.devforge/memory.md`, the project's persistent cross-session lessons file). **Carry `memory_excerpt` forward to Phase 3.1**, which appends the entries bearing on the scope to the agent context file. The pitfalls, past incidents, and recurring lessons it records are the same shape as the recurring-issues mapping Phase 4.3 performs, so an entry naming a defect this codebase has already been bitten by is exactly what the finders should hunt for. When `memory_present` is false, or the excerpt holds only the shipped stub's headings with no entries under them, carry nothing forward: say nothing to the user about memory, raise no warning, add no step. A project with no recorded lessons is the ordinary state of a fresh install, not a fault to remedy. The excerpt is the first 40 lines, not the whole file — an entry's absence from it means "not in the first 40 lines", never "never recorded".
