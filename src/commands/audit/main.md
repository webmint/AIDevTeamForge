...
- `$WORKDIR/context.md` — the optional `--extra-context-file` payload (constitution rules, `.devforge/memory.md` pitfalls, recurring list). Written by the orchestrator in Phase 3.1, read by every `render-agent-brief --extra-context-file`.
...
- `$WORKDIR/recurring.json` — `[{file, fingerprint}]` past-review findings the orchestrator extracts from the recent feature `review.md` reports Phase 4.3 Step A locates. Written in Phase 4.3 (broad + hotspot + directory/uncommitted; NOT single-file), read by `map-recurring-issues --recurring`.
...
Detects which of the four audit-capable agents exist in `.claude/agents/`: `code-reviewer`, `architect`, `qa-reviewer`, `security-reviewer`. The result is always JSON on **stdout** — `present` + `missing` lists plus an `all_missing` boolean; nothing is written to stderr. The verb exits **3** when `all_missing` is true (zero agents installed): copy the stdout JSON VERBATIM as a fenced block and end the turn — the user must restore the agent files first. `update.sh` restores an agent only when it was never installed in this project; agent files deleted after they were installed come back from `install.sh` (which regenerates every agent, and also overwrites `.claude/settings.json`) or from hand-copying them into `.claude/agents/`. When 1–3 exist (exit 0), proceed and carry the `missing` list forward for the report's "Agents skipped (not installed)" section.
...
Reads `constitution.md`, `CLAUDE.md` (Source Root, project type, framework, language), and `.devforge/memory.md` (pitfalls, past incidents, lessons), and emits a structured context block on stdout for downstream phases. This verb is best-effort and ALWAYS exits 0 — the constitution guard is a JSON-field check, NOT an exit code: if the stdout JSON has `"constitution_populated": false` (the file is absent or still contains a populate-marker), STOP — tell the user VERBATIM "⛔ constitution.md has not been populated yet. Run `/devforge:constitute` before using `/devforge:audit`." and end the turn.
...
The same stdout JSON carries `memory_present` (bool) and `memory_excerpt` (the populated `## ` sections of `.devforge/memory.md`, the project's persistent cross-session lessons file, with `## Task Outcomes` excluded and every section that has no entries under its heading dropped). **Carry `memory_excerpt` forward to Phase 3.1**, which appends the entries bearing on the scope to the agent context file. The pitfalls, past incidents, and recurring lessons it records are the same shape as the recurring-issues mapping Phase 4.3 performs, so an entry naming a defect this codebase has already been bitten by is exactly what the finders should hunt for. When `memory_present` is false or `memory_excerpt` is empty — the shipped stub carries no entries under its headings, so it renders as an empty excerpt — carry nothing forward: say nothing to the user about memory, raise no warning, add no step. A project with no recorded lessons is the ordinary state of a fresh install, not a fault to remedy. The excerpt is not the whole file — when the line budget cannot fit a section whole, the lines it drops are always that section's EARLIEST ones, with an inline marker line right after the heading naming how many were omitted, so an entry's absence from a non-empty excerpt means it sits in the excluded section or behind a marker the excerpt itself declares, never "never recorded".
...
- `Risk-targeted sample` — re-run as `/devforge:audit --top 25` (recommended for periodic checks).
...
`render-agent-brief` reads `$WORKDIR/scope.json` and the reference files under `--references-dir` (default `.devforge/command-refs/audit` — leave it unset, that is the installed location). `--finding-cap` (default 30) is substituted into the output contract + closing reminder wherever the cap is named. `--tmp-path PATH` sets the EXACT path the brief tells the agent to write its findings to; pass `"$WORKDIR/tmp-<agent>.md"` so the temp lands in `$WORKDIR` (outside `audits/`, reaper-immune). If `--tmp-path` is omitted the brief defaults to the legacy `audits/.tmp-<agent>.md` location, so pass it every time. It assembles the structured brief in this order: the adversarial preamble, the mislogic checklist, the best-practices checklist (all three read verbatim from the reference files), the agent-specific focus block, the scope block (plus any `--extra-context-file` content appended to it), the output contract, and the closing mode reminder. The closing reminder is the LAST instruction in the brief so the most-recent instruction wins over the agent's baked-in polite tone.

**Constitution excerpts via `--extra-context-file` — standard for `/devforge:audit`.** The best-practices checklist's "Constitution-principle adherence" hunt only works when the constitution rules are present in the agent's brief — an agent cannot check the code against principles it cannot see. So the orchestrator SHOULD assemble a context file containing the project's constitution rules and pass it via `--extra-context-file <path>` to every agent, so each can hunt constitution-principle violations and tag them `[CONSTITUTION-VIOLATION]`. `preflight-context` (Phase 1.3) only reports whether the constitution is populated (the `constitution_populated` flag), NOT its text — so the orchestrator reads `constitution.md` directly (it lives at the workspace root, the CWD; Phase 1.3 confirms it is populated) and writes the relevant rules to a scratch file `$WORKDIR/context.md` (outside `audits/`, swept by the end-of-run `rm -rf "$WORKDIR"`), then passes that path via `--extra-context-file "$WORKDIR/context.md"`. When no constitution rules reach the brief, agents report none from the constitution-adherence section (per the checklist), and the rest of the full-spectrum hunt is unaffected. The recurring-issues list remains an optional addition to the same file.

**Known pitfalls via the same context file.** When Phase 1.3 carried `memory_excerpt` forward (it had entries), append those of its entries that bear on the files in scope to the SAME `$WORKDIR/context.md`, under their own heading so they are not mistaken for constitution rules. This is what points the finders at what this codebase has already been bitten by — a defect recorded once and re-introduced is precisely the recurrence the full-spectrum hunt exists to catch. When Phase 1.3 carried nothing forward, append nothing: the context file then holds the constitution rules alone, exactly as before, and no agent is told anything about memory. **Honesty bound.** A memory entry is an UNVERIFIED prior-session assertion, not evidence — it tells a finder where to LOOK, never what to REPORT. Every finding still stands or falls on the verbatim source quote `validate-findings` (Phase 4.1) grounds it against; a finding whose only support is a memory entry has no grounding and is discarded there. The code an entry describes may have changed since it was written, or the entry may have been wrong when written.
...
To avoid the context-exhaustion failure mode (CHANGELOG 1.27.0 for `/devforge:verify`), dispatch in two batches, not all four at once. Each batch is multiple Task calls issued in a single turn (true parallel); wait for both to complete before the next batch.
...
```bash
WORKDIR="${TMPDIR:-/tmp}/forge-audit"
.devforge/lib/audit_helper render-verify-brief --findings "$WORKDIR/refute-<refuter>.json" --refuter <refuter> --references-dir .devforge/command-refs/audit --scope "$WORKDIR/scope.json" --source-root <source-root> --tmp-path "$WORKDIR/verdicts-<refuter>.md"
```
...
**Step A — build the recurring payload.** Locate the past feature reviews:

```bash
.devforge/lib/artifact_helper find-feature-artifacts --filenames '["review.md"]' --limit 5
```

`find-feature-artifacts` walks every feature directory the install has and reports the `review.md` sitting directly in each one; a `review.md` inside a nested subdirectory is not listed. `--limit 5` caps the result to the 5 most recent records, applied AFTER the recency ordering is computed — so the survivors are chosen from every review in the install, never from a pre-truncated slice. The cap counts match RECORDS, not directories, but a feature directory can hold at most one `review.md`, so five records are five distinct features' reviews — exactly the "5 most recent reviews" this step wants.

Stdout is a JSON object; take `matches_by_recency` from it — the newest-first ordering, at most 5 entries under `--limit 5`. Each entry carries `file` (that review's own path, ready to read) and `mtime_iso` (its last-modified UTC timestamp, `YYYY-MM-DDTHH:MM:SSZ`). Derive the 90-day cutoff ONCE — the same today's-date `YYYY-MM-DD` value Phase 4.5 uses for `audit_date`, minus 90 days — and drop every entry whose `mtime_iso` date (its leading `YYYY-MM-DD`) is earlier than that cutoff; the entries that survive are the qualifying reviews. The cap and the window are complementary, and applying the window AFTER the cap is not an ordering to "fix": the window keeps exactly the newest records, so filtering the 5 newest overall yields the same set as taking the 5 newest from among the in-window ones. An EMPTY `matches_by_recency` means no feature directory holds a `review.md`: the call exits 0 in that case, because finding nothing is a normal outcome and not a failure, so there is no exit code to test for it — branch on the array being empty. A non-zero exit means the call itself failed (a malformed flag value, or a workspace that could not be resolved), never that no review exists: copy the helper's stderr VERBATIM into your next user-facing message as a fenced code block (do not summarize or paraphrase), then end the turn.

Read each qualifying review and extract its Critical findings ONLY (cap 25 total across all reviews). Write them to `$WORKDIR/recurring.json` as a `[{file, fingerprint}]` list — `file` there is the SOURCE file the past finding named, the value `map-recurring-issues` substring-matches against a current finding's own `file`; it is NOT the review's path, so do not carry a match record's `file` into this key. If no reviews qualify, write `[]` and note "No recent reviews to cross-reference." in the eventual summary. Track which review files you consulted — their `file` values are what the report dict's `recurring_reviews_consulted` lists.
...
It maps each past finding against the working list — RESOLVED / RECURRING / RECURRING-SPREAD — by exact match, tags matched findings, and bumps their severity. This is the audit's differentiator over `/devforge:review`: it sees drift across features. Stdout (captured to `$WORKDIR/recurring-mapped.json`) is a DICT carrying `findings` (the working list, now recurring-tagged) and `recurring_status` (a `[{past, status}]` list). Derive the report dict's `recurring_resolved` / `recurring_unresolved` by splitting `recurring_status` on `status` (`RESOLVED` → resolved; `RECURRING` / `RECURRING-SPREAD` → unresolved). Algorithmic merging only — exact-match keys in the helper, never LLM semantic judgment. Extract the recurring-tagged `findings` array into a bare array for 4.4:
...
2. **Standalone** — `/devforge:audit` is never invoked by another command, never part of any chain, never auto-triggered.
