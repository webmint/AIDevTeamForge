...
- `references/triage.md` — how to triage a working-list item (defect-repair vs feature/architecture change) and the PHASE-1 D7 `/specify` bounce criteria. Read it in full at PHASE 1.
...
7. **Writes only a `[WIP]` commit** — `/fix` writes NO report, mutates NO spec/task/`review.md`/`verification.md`, and writes or closes NO `bugs/` file (D4 — filing a bug is the separate "defer" arm). Its only durable output is the remediation `[WIP]` commit, squashed by `/finalize`.
