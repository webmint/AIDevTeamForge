...
8. **Wrapper-mode aware** — the `source_root` from `preflight` drives `resolve-feature-scope --source-root`, `verify-touched --root`, and `check-hygiene --source-root`; `specs/[feature]/`, `bugs/`, and `verification.md` always live at the workspace root.
