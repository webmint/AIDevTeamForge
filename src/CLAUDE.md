...
- `/devforge:research "topic"` — Investigate a bug or enhancement against the existing codebase (required intake lane for `/devforge:specify`); on a confirmed save allocates `specs/NNN-name/` + the `spec/`-prefixed feature branch and writes `research-report.md` + `research-handoff.json` there, WIP-committing them as it writes (folds into `/devforge:finalize`'s squash)
- `/devforge:discover "feature idea"` — Greenfield-feature discovery (required intake lane for `/devforge:specify`); on a confirmed save allocates `specs/NNN-name/` + the `spec/`-prefixed feature branch and writes `discovery-report.md` + `discover-handoff.json` there, WIP-committing them as it writes (folds into `/devforge:finalize`'s squash)
