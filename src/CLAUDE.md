...
Authors a structured 9-section feature spec at `specs/NNN-<feature-name>/spec.md`. Hard-gated on the 4-command setup chain (`/init-forge` → `/generate-docs` → `/configure` → `/constitute`) — refuses to run when any artefact is missing or `constitution.md` still contains the populate-guard literal. Phase 0 preflight + branch detection + `.claude/session-state.md` reset. Phase 1 reads 7 mandatory input sources (constitution + MEMORY + CLAUDE + docs/ + research/ + discover/ + specs/) with path-based source tagging (no content parsing of upstream `/research` or `/discover` reports). Phase 1.5 enumerates structured findings per source as a REQUIRED INTERMEDIATE OUTPUT (≥3 bullets or explicit "no items relevant" marker). Phase 2 surfaces decision points across 7 categories (`scope_boundaries`, `existing_behavior`, `data_flow_state`, `edge_cases`, `ui_ux_details`, `breaking_changes`, `tooling_configuration`); C-strict auto-vs-interactive mode detection (env var `DEVFORGE_AUTO_MODE=1`, `--auto` flag, or `<system-reminder>` substring — user prose ignored); interactive uses AskUserQuestion (single-line) with markdown fallback; auto records `[default applied]`; per-DP 3-follow-up turn cap. Phase 3 classifies the spec into 5 types (`migration_tooling` / `feature_addition` / `bug_fix` / `refactor` / `greenfield_feature`) with `/discover` path pre-seeding `greenfield_feature`; per-type mandatory-read slot tables + CBM / Glob / Grep discretionary exploration + MEMORY.md cross-reference. Phase 4 renders the 9-section spec via setters with 7-subsection AC categorization, EARS-validated statements (5 variants — §5.1 + §5.7 Ubiquitous-only + verification-command required), Phase 1.5 coverage rule (every finding lands in AC / Constraint / OOS / Risk), numerical-consistency check, non-blocking constitution-recheck. Phase 5 hard-gate approval via deterministic 4-bullet summary; on approve, `finalize-handoff` writes `specs/NNN-<feature-name>/handoff.json` (specify→plan structured handoff, status stays `Draft`) + a manual-next-step `/plan` block (`/plan` auto-discovers this handoff on its first run and reads the upstream plan-seeds; the user still launches `/plan` manually — restarts Claude Code, copies the embedded `/plan specs/NNN-feature-name/spec.md` line, as there is no auto-dispatch). State at `.devforge/specify-state.json`. **Requires approval before proceeding.** Auto-creates a `spec/NNN-short-desc` branch when on the default branch.
