...
#### `/breakdown [plan-file]`
Takes an approved plan and generates ordered, atomic tasks with dependencies, agent assignments, and verifiable Expects/Produces contracts. Each assigned agent is validated against the installed `.claude/agents/` roster (a Phase 3.5 hard gate) so a task can never be assigned an agent the project doesn't have. Saves task files to `specs/[feature]/tasks/` and writes a structured `specs/[feature]/breakdown-handoff.json` (the producer side of the breakdown→`/implement` handoff). WIP-commits the task files + handoff as it writes them (folds into `/finalize`'s squash). **Requires approval before execution.**
