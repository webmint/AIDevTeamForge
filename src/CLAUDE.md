...
- Structural queries (exports, types, callers, deps, dead code) are NOT in docs/ — query the codebase-memory-mcp graph live via MCP tools (`search_graph`, `trace_call_path`, `get_code_snippet`, `agentic_context`, `semantic_query`)
- Md files are auto-indexed by codebase-memory-mcp; `agentic_context "<fuzzy topic>"` returns md narrative + code structure together
