...
#### `/plan [spec-file]`
Takes an approved spec and produces a technical plan: architecture decisions, data model, API contracts, research. Saves to `specs/[feature]/plan.md`. WIP-commits the plan + handoff as it writes them (folds into `/finalize`'s squash). **Requires approval before breakdown.**
