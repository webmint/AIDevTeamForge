...
**Styling is normally grounded in the existing component library and any design source — that stance is UNCHANGED.** This rule adds two obligations: a universal built-UI sanity floor that always applies, and — when a design source is captured as a first-class anchor — a fidelity obligation against that captured intent.

- **Every built UI meets a sanity floor, no design source needed.** No unintended horizontal overflow, no unintended clipping of content out of its container, and every declared font actually loads (a declared-but-unloaded font silently renders a fallback). These are self-evident defects that need no design reference to detect.
- **When a design source is captured, the built UI matches that captured intent.** The built element's rendered VALUES — color, spacing, radius, typography, and font-family — AND its box geometry (width and height) must equal the captured design intent. Absolute page position is out of scope: the design source renders standalone in its own document, so its page coordinates share no reference frame with the built app — only box dimensions are compared, not where the box sits on the page. Markup may be rebuilt freely with clean semantic classes; only the rendered result must match. "Semantically equivalent markup" is not a defense for a wrong color, wrong spacing, or a missing hover/focus state.

**Design intent is a first-class, captured input — not re-derived at each stage.** It has two parts:

- A **design anchor** — the design source plus which of its elements carry the intent — captured at intake and persisted with the feature.
- A **binding** — a render `route` plus `{ anchor_selector ↔ built_testid }` pairs — authored at breakdown, naming where the feature renders and which built element corresponds to which intent element.
...
- **Static provenance check (write-time, reads source text).** Component styles use no hardcoded color literals (no hex / `rgb()` / `hsl()` / named colors), no `var(--x, <literal>)` fallbacks, and bind to defined tokens; an undefined token must FAIL LOUDLY rather than silently render a fallback; interactive elements declare both `:hover` and `:focus-visible`. (A bare hardcoded spacing or border value that never invokes `var()` — e.g. `padding: 16px` — is NOT caught by this static check; the `var(--x, <literal>)` and undefined-token rules fire only when a `var()` is actually used.) These checks read only source text — they are independent of the anchor and run on every UI change. Reading source text alone CANNOT catch a correct token binding that still produces the wrong rendered result.
- **Runtime conformance check (reads rendered/computed values).** At review time the `design-auditor` runs a two-tier check against the running UI: the always-on sanity floor above, plus — when an anchor is captured — a value-and-geometry comparison. That comparison reads the built UI's computed values and geometry and diffs them against the ACTUAL design source, rendered and measured through the anchor's captured elements (not against a proxy value source). Reading resolved values with no provenance CANNOT catch a token bypass — a hardcoded `#e8e6e3` and a `var(--border)` that resolves to `#e8e6e3` are indistinguishable once rendered.
...
**The runtime check reports NOT-COVERED, loudly, rather than a false PASS whenever it cannot actually run** — no Chrome DevTools MCP, a non-web platform, an unimplemented anchor kind, an absent render route or region, or no captured anchor (in which case only the anchor-gated fidelity layer is not covered, while the always-on sanity floor still runs). A PASS means a real check executed against a found region — never "nothing was checked."

*Backed by* `constitute_helper verify-design-tokens` when `forcing_functions.design_token_provenance.enabled = true` in `.devforge/constitute.json` — hardcoded color literals in component style files (plus `var(--x, <literal>)` fallbacks, undefined tokens, and interactive elements missing `:hover` / `:focus-visible`) surface as exit-2 findings when `constitute_helper verify-design-tokens` is run directly or via the optional `.devforge/templates/git-hooks/pre-commit-forcing-functions.sh` hook. A bare hardcoded spacing or border value that never invokes `var()` is not among these findings.
