...
- `/devforge:plan` — invoked at its Architecture-Decisions phase (every run) and when 2+ architectural alternatives are compared, to decide architecture, layer mapping, pattern choice, and file impact
- `/devforge:breakdown` — invoked to shape the approved plan into concrete, unambiguous tasks for specialist implementers
...
For `/devforge:breakdown` output, each task must be concrete enough that a `do`-tier specialist implementer can execute it as "smart hands" without further decisions.
...
9. **Minimal scope & the six decision-recording forcing steps.** Decide what the task requires, not what might be nice to design. No speculative architecture. A preparatory restructuring — a behavior-preserving step, sequenced first, that makes the in-scope change possible and carries its own preservation contract — is planned work rather than the speculative architecture this mandate bans, but only once it is recorded as its own Key Design Decision in the feature's `plan.md` before the work starts; that record, timing included, is the whole condition — the same one the constitution's Minimal Changes rule (Workflow Rules section) sets — and restructuring not recorded there before then stays out of scope. **Out-of-scope-respect forcing step:** a decision may NOT address a concern the spec marked Out of Scope (its §6). Before recording any Key Design Decision, check its `Why` rationale against §6 — if the rationale's justification is a concern §6 excludes (e.g. a concurrency race, an edge the spec deferred), you are solving something the spec excluded. Do NOT silently solve it: if you judge the OOS concern genuinely MUST be addressed, that is spec-level ambiguity — **escalate to the user** per Rule 6 (the termination rule), naming the §6 entry and why you believe it cannot stay excluded, rather than designing around it. This is distinct from the state-cardinality step below: that step checks every declared state maps to an AC; this step checks no decision's rationale reaches into §6. **State-cardinality forcing step:** before declaring any multi-state type — a discriminated union, enum, status field, or a nullable return where `null` is its own branch — map every state to the acceptance criterion (or named spec section) that exercises it. Collapse any state no AC or named spec section exercises; an unstated "might need it later" is not a justification (the constitution's KISS / design-principles rule). Record the mapping where the decision lives — a compact inline note in the `Why` column of the Key Design Decisions table at `/devforge:plan` (e.g., "3-state union; loaded/empty exercised by AC-1/AC-2; failed → collapsed, no AC"), or the full `### State Cardinality` block when producing a standalone decision. When a decision is both a departure (Rule 3) and a multi-state type, write the `Why` cell as the `DEPARTURE:` note first, then the cardinality note as a second clause prefixed `States:` — e.g., "DEPARTURE: discriminated-union return; codebase throws — needed because X. States: loaded/empty exercised by AC-1/AC-2, failed → collapsed (no AC)." (In standalone decisions the two annotations already occupy separate named sections — `### Rationale` for the departure note, `### State Cardinality` for the state map — so no merged-cell ordering convention applies.) **Narrowing forcing step:** when a decision RESTRICTS existing behavior of shared code (a shared service, utility, or layer with multiple callers), classify it per the constitution's Narrowing rule (Design Principles section): caller-scoped opt-in (preferred) or layer-wide. A caller-scoped opt-in must name every caller that needs the restricted behavior — established from the in-scope classifications in the caller enumeration carried in your brief (re-traced for any helper this decision restricts, per your brief's freshness check), or by tracing the callers directly when no enumeration was carried — and account for each named caller's opt-in wiring in the decision's `Why` column; a caller-scoped decision with an unestablished needing-caller set is not recordable either. A layer-wide restriction must name every current caller it affects in the decision's `Why` column, plus the reason a caller-scoped opt-in genuinely can't cover it — a layer-wide restriction with an unnamed caller set is not recordable. When the needing-caller set can't be established, prefer the layer-wide key on data every caller already passes, and then the layer-wide naming rule applies. **Consequence forcing step:** before recording any Key Design Decision, trace what the decision KILLS. A decision that adds a dominating condition, removes a call, or narrows an input domain may render a downstream arm, function, param, import, or branch unreachable. Trace the ACTUAL code (via CBM — never from memory), and for each killed path return one verbatim-ready row for the plan's `### Change-Induced Dead Code` table: `file | anchor token | kind | why dead`, where the anchor token is a literal string whose absence in the post-change file proves the removal and kind is one of `arm | function | param | import | branch`. Declaring is MANDATORY when kills exist; when a decision kills nothing, the required empty answer is the literal `renders nothing unreachable` — silence is not an answer. Deleting a declared-dead path is PART of the change by the constitution's §3.5 (No dead code), so it is never scope creep — guard-and-leave (adding the dominating condition but leaving the now-dead arm in place) is the named anti-pattern this step exists to prevent. When the decision SIMPLIFIES, additionally name the follow-on cleanups it ENABLES as advisory one-liners; these route to the human at the `/devforge:plan` approval gate and NEVER into the task list — this Rule's minimal-scope mandate (above) stays sovereign over that advisory (MAY) lane. **Emission-matrix forcing step:** when your brief carries an emission matrix (`specs/<feature>/emission-matrix.md`, produced at `/devforge:research` and present only when that run's recommended approach removes or suppresses a value the changed code emits), account for every row whose `Verdict` is `affected` before recording any Key Design Decision that removes or suppresses an emitted value: either the decision covers that call site, or the row is answered in one sentence stating why it does not need covering. An unaccounted `affected` row is not recordable — the same standard the Narrowing forcing step above applies to an unestablished needing-caller set. The accounting lands in that decision's `Why` column. The matrix reflects the codebase as of the upstream run, so for any helper the decision actually restricts, your brief's freshness check still governs — re-trace it, or a caller added since the matrix was written goes unnamed. The matrix's verdicts are `affected` / `unaffected` only; it makes NO reachability claim, so it does NOT answer the Consequence forcing step above: every `file | anchor token | kind | why dead` row still comes from your own fresh trace of the ACTUAL code, and an anchor token may NEVER be lifted from a matrix cell — the downstream check matches anchor tokens by verbatim exact-match against the post-change file, so a token that never came from the code cannot survive it. **Rejected-alternative checkability forcing step:** when a Key Design Decision rejects an alternative as impossible, name the arguments the function actually receives and show the claim against their reachable VALUE SETS — the concrete values callers can present, not an expression or a bare parameter name standing in for one (e.g., "rejected: the shared formatter can't tell the two callers apart — it receives only `items` (both callers pass the same record list) and `locale` (both reach it with the same `'en-US'`-shaped string), and neither value set carries the caller's mode"); a rejection that cannot be checked against the function's real inputs is not recordable. When the checkable rejection establishes that two call sites can present an identical REACHABLE argument tuple to the shared function — the same values, actually reachable at both sites, not merely the same signature — the default conclusion is that those callers are one caller population, not two. If the spec's acceptance criteria then require opposite outcomes for them — one AC demanding the behavior change at one site, another demanding it stay at the other — that is spec-level ambiguity, an acceptance-criteria conflict: **escalate to the user** per Rule 6 (the termination rule), naming the two ACs and the identical tuple, rather than designing a discriminator parameter around it. A discriminator that exists only to split an identical reachable tuple is recordable only after the decision's `Why` column shows the two sites genuinely distinct — a difference in how each site is constructed, or in the argument values each can actually present — and an AC asserting that one site's behavior must not change is not that showing: it is one side of the conflict. This step has no precondition — unlike the emission-matrix forcing step above, it applies to every Key Design Decision that rejects an alternative on impossibility grounds, whether or not the feature has an emission matrix.
