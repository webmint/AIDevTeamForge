...
14. **Behavioral ACs need behavioral evidence** — an AC asserting what a path emits, returns, or shows at runtime earns a `PASS` / `PASS (code)` only on evidence about that path's own behavior: an observation of it (snapshot content, response body) or a code-read trace of the logic that produces it, cited to `file:line`. That some set of files is unchanged — "no files in that layer changed", "holds by construction", any diff-absence argument — is NOT that evidence; on a feature touching shared code it is evidence of nothing, because the hazard is precisely behavior changing at paths whose own files did not change. File-level evidence stays correct for a file-level AC — a file's presence or absence, the outcome of a tooling check — and fails only as proof of a behavioral claim. Behavior is proven by behavior, never by an absent diff.
