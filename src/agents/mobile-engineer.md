...
- **Carry the design binding's test tag (framework build requirement, not a constitution rule).** When the feature has a design binding (`specs/[feature]/design-manifest.json`), set the platform's stable test/identity tag — a test tag or accessibility identifier — equal to the binding pair's `built_testid` on the region or screen element you build, so the review-time design-fidelity check locates that built element by its stable tag rather than by fuzzy matching. The tag goes on the containing region — and on any element the binding pairs individually — never as a per-element anchor mirroring reference ids. Caveat: the review-time built-reader is a DOM probe (Chrome DevTools `querySelector` on the testid + computed style), so only a DOM-inspectable build (web, or a DOM-based mobile shell such as Capacitor / Ionic / React Native Web) actually consumes this tag today; for a genuinely native, non-DOM build no built-reader exists yet — this tagging is forward-compatible prep, not an active check today, and the review-time gate reports NOT-COVERED for a non-web platform regardless.
