...
Owns the shape of `.devforge/configure.yaml`: 28 fields covering project
metadata, language/framework stacks, project natures (for agent pruning),
build/lint/type-check commands, per-package stack records, workflow
enforcement, AI attribution, Claude tier settings, and AC verification
parameters. `/configure` is the third command in the 4-command pivot
...
all 28 setter subcommands (set-project-name, set-project-description,
set-project-type, set-primary-language, set-languages, set-frameworks,
set-architectures, set-project-natures, set-error-handlings, set-api-layers,
set-testings, set-build-tools, set-build-commands, set-type-check-commands,
...
with all 36 keys — 28 from configure.yaml + 5 from init.yaml + 3 derived),
...
Note on key count: project-config.json contains 36 keys. The breakdown is:
  28 from configure.yaml (FIELD_SCHEMA fields → UPPERCASE keys)
...
renders all 36 keys as the correct contract.
...
Step 5a helper plumbing: prune-agents subcommand reads project_natures
from configure.yaml, walks .claude/agents/*.md, parses applies_to
frontmatter, and emits a JSON report of kept/dropped decisions. With
--apply, deletes dropped agent files. Without --apply, dry-run only.
...
- FIELD_SCHEMA contains 28 fields in locked order (project_name,
  project_description, project_type, primary_language, languages,
  frameworks, architectures, project_natures, error_handlings, api_layers,
  testings, build_tools, build_commands, type_check_commands, lint_commands,
...
- `default_state()` walks FIELD_SCHEMA and returns all 28 keys with
...
- All 28 setters route through `_state_transaction(devforge_dir)` which
...
# ---------------------------------------------------------------------------
# Step 5a: prune-agents implementation.
# ---------------------------------------------------------------------------
