...
def _plan_handoff_block(state: Dict[str, Any]) -> str:
    ...
    return (
        ...
        "/plan, but /plan's auto-discovery reader is not yet wired — so run "
        "the command below manually. Restart Claude Code (exit and relaunch "
        "the CLI/app so the newly installed command is picked up), then run "
        "it in this repo. The spec path is explicit so /plan does not need "
        "most-recent-spec discovery:\n"
        "\n"
        "~~~\n"
