...
def _glob_plans(cwd: str) -> List[str]:
    ...
    function (cmd_pick_plan / cmd_list_plans) re-sort the returned paths
    by mtime, so the accessor's own ordering is not load-bearing here --
    only membership is.
    ...
    result = []
    for feature_dir in find_feature_dirs_with(specs_root, "plan.md"):
        result.append(str((feature_dir / "plan.md").resolve()))
    return result
